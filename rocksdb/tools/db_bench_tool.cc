...
uint64_t trace_b_value=95;
int test_fillseq = 0;
int test_overwrite = 0;
int test_readrandom = 0;
int test_readseq = 0;
std::mutex rd_lock_;
std::mutex wr_lock_;
...
class KVComp;
class WorkerQueue;
extern KV_SSD * kv_ssd_;
extern KV_SSD * kv_ssd_new_;
extern KVIndex * kv_index_;
extern KVComp * kv_comp_;
extern WorkerQueue * kv_worker_;
//extern KV_SSD kv_ssd_;
extern bool du_enable_;
extern bool kvssd_compaction_enable_;

extern int kvssd_count_;
extern int kv_ssd_enable_;
extern int kv_comp_opt_;
extern int kv_seq_opt_;
extern int kvssd_algo_value_;
extern bool du_trivial_disable_;
uint64_t time_duration_ = 6000;
extern uint64_t du_kv_range_;

extern float rocks_avg_[16];
extern float kv_avg_[16];
 
extern int kv_ssd_threshold_ratio_;
extern long long int total_rocksdb_size_; // MB
extern long long int total_kv_range_; // MB
extern int  warmup_phase_;
...
bool warm_done[MAX_THREAD_NUM] = 
    ...
    true, true, true, true,
    true, true, true, true};
// 0: Nvme only 1; KV sSD only 2: mixed 3: Our Work  
DEFINE_int64(kv_ssd_enable, 0, "KV SSD only or ");  
DEFINE_int64(kv_ssd_max_value, 0, "KV SSD only or ");  
DEFINE_int64(kv_ssd_mix_max_value, 8192, "KV SSD only or ");  
DEFINE_int64(kv_ssd_mix_min_value, 256, "KV SSD only or ");  
DEFINE_int64(kv_comp_opt, 0, "KV SSD only or ");  
DEFINE_int64(kv_seq_opt, 0, "KV SSD only or ");  
DEFINE_double(kv_ssd_zipf_dist, 0.99, "DetComp zipf distribution, default 0.99");
DEFINE_uint64(kv_ssd_key_range, 8, "DetComp key range");
DEFINE_bool(kv_ssd_value_change, false, "DetComp key range");
DEFINE_uint64(kv_ssd_value_small, 16, "DetComp key range");
DEFINE_uint64(kv_ssd_value_medium, 4, "DetComp key range");
DEFINE_uint64(kv_ssd_value_large, 1, "DetComp key range");
DEFINE_uint64(kv_ssd_count, 1, "DetComp key range");


DEFINE_uint64(kv_ssd_value_small_size, 256, "DetComp key range");
DEFINE_uint64(kv_ssd_value_medium_size, 1024, "DetComp key range");
DEFINE_uint64(kv_ssd_value_large_size, 4096, "DetComp key range");


DEFINE_bool(kv_ssd_du_enable, false, "DetComp key range");



// For ATC Performance Mode
DEFINE_bool(kv_ssd_perf, false, "KV SSD Performance Mode");
DEFINE_int64(kv_ssd_first_value, 256, "KV SSD Performance Mode");
DEFINE_int64(kv_ssd_value_ratio, 4, "KV SSD Performance Mode");
DEFINE_int64(kv_ssd_value_count, 3, "KV SSD Performance Mode");
DEFINE_int64(kv_ssd_distribution, 1, "KV SSD Performance Mode");
DEFINE_int64(kv_ssd_db_size, 1, "QWER");
...
//DEFINE_uint64(kv_ssd_bench_type, 0, "DetComp key range");
uint64_t kv_ssd_bench_type = 0;
// 0 : seq 1: rand  2: zipf 3: Mix
int kv_ssd_pattern = 0;
int kv_ssd_threads = 0;
bool process_fill = false;

bool only_write = false;

int * kv_ssd_value_list_;
uint64_t kv_ssd_qwer_cnt_;
uint64_t kv_ssd_key_range_;
uint64_t kv_ssd_thread_gap_;
int * value_list;// = (int *) malloc(sizeof(int) * FLAGS_kv_ssd_value_count);
int * freq_list; // = (int *) malloc(sizeof(int) * FLAGS_kv_ssd_value_count);
int freq_sum;

#if (0)
KV_SSD * kv_ssd_new;
/********************************* TEST REGION New KV SSD *****************************************/
void CreateNewKVSSD() {
    if (FLAGS_kv_ssd_count != 1) {
        printf("Create one More KVSSD\n"); 
        kv_ssd_new = new KV_SSD(12);
    } else {
        printf("Do not need to create more KVSSD\n");
    }
}
/********************************* TEST REGION New KV SSD *****************************************/
#endif
...
bool key_shuffle_done = false;
std::vector<uint64_t> key_seq; // key shuffler
std::vector<uint64_t> key_latest; // key shuffler
std::vector<int> value_size_seq; // value_size 
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
  ...
  uint64_t get_zipf(void) {
      ...
      double u = (double)(gen2() % RAND_MAX) / ((double)RAND_MAX);
      double uz = u * zeta_n;
      if (uz < 1.0) {
          return base;
      }
  ...
  uint64_t get_seq_key(ThreadState* thread) {
      uint64_t ret = (thread->start_key_num % kv_ssd_key_range_) + 1;
      //uint64_t ret = (thread->start_key_num % FLAGS_kv_ssd_key_range) + 1;
      thread->start_key_num = ret;
      return ret; 
  }
  uint64_t get_key(int tid) {
      if (1) {
          int new_index = get_zipf();
          int real_index = (new_index + tid * kv_ssd_thread_gap_) % kv_ssd_key_range_;
          //printf("Tid %d new_index %d real_index %d\n", tid, new_index, real_index);
          return key_seq[real_index];
      } else {
          return key_seq[get_zipf()];
      }
  }

  void put_latest(uint64_t number) {
      wr_lock_.lock();
      bool dbg_print = false;
      if (rand () % 100 == 0) dbg_print = false;
      if (dbg_print) 
          printf("put latest %lld latest size %ld\n", (long long int) number, key_latest.size());
      auto it = find (key_latest.begin(), key_latest.end(), number);
      if (it == key_latest.end()) {
        key_latest.push_back(number);
      } else {
          key_latest.erase(it);
          key_latest.push_back(number);
      }

      if (key_latest.size() > 80000) {
          key_latest.erase(key_latest.begin());
      }
      if (dbg_print) 
      {
          for (int i = 0 ; i < 10 ; i ++) {
              int new_index = (int) key_latest.size() - 1 - i;
              if (new_index >= 0) {
                  printf("put list %d : %lld\n", i, (long long int) key_latest.at(new_index));
              }
          }
      }
      wr_lock_.unlock();
  ...
  uint64_t get_latest() {
      ...
      //7000 개라고 가정 함 zipf return -> 0 이면 7천 나와야 함. 
      int new_index = 0;
      new_index = (int) get_zipf() % key_latest.size();
      if (dbg_print) printf("zipf %d\n", new_index);
      
      new_index = key_latest.size() - 1 - new_index;
      if (dbg_print) printf("in this ...%d\n", new_index);
      
      //new_index = (key_latest.size() - 1 - (int)(index % key_latest.size()));
      //if (dbg_print) printf("last_index %d\n", new_index);
      
      ret_val = key_latest.at(new_index);
      if (dbg_print) printf("ret_val  %d\n", (int) ret_val);
      
      wr_lock_.unlock();
      return ret_val;
      //return key_latest[get_zipf()];
  }
  
  void MakeTrace() {
      key_latest.reserve(kv_ssd_key_range_);
      key_seq.reserve(kv_ssd_key_range_);
      //key_seq.reserve(FLAGS_kv_ssd_key_range);
      for (uint64_t i = 0; i < kv_ssd_key_range_; i ++) {
      //for (uint64_t i = 0; i < FLAGS_kv_ssd_key_range; i ++) {
          key_seq[i] = i + 1;
      }
      
      //for (uint64_t i = 0; i < FLAGS_kv_ssd_key_range; i ++) {
      for (uint64_t i = 0; i < kv_ssd_key_range_; i ++) {
          uint64_t r = (uint64_t)rand() % (kv_ssd_key_range_ - i) + i;
          //uint64_t r = (uint64_t)rand() % (FLAGS_kv_ssd_key_range - i) + i;
          uint64_t temp = key_seq[i];
          key_seq[i] = key_seq[r];
          key_seq[r] = temp;
      }

      printf("zipfian distribute value %f\n", FLAGS_kv_ssd_zipf_dist);
      items = kv_ssd_key_range_;
      //items = FLAGS_kv_ssd_key_range;
      base = 0;
      zipf_dist = FLAGS_kv_ssd_zipf_dist;
      theta = zipf_dist;
      zeta_2 = zeta(0, 2, 0);
      alpha = 1.0 / (1.0 - theta);
      //zeta_n = zetastatic(0, FLAGS_kv_ssd_key_range, 0);
      //num_zeta = FLAGS_kv_ssd_key_range;
      zeta_n = zetastatic(0, kv_ssd_key_range_, 0);
      num_zeta = kv_ssd_key_range_;
      eta = (1 - pow(2.0 / items, 1 - theta)) / (1 - zeta_2 / zeta_n);

      get_zipf();
  }
  
  OperationType get_req_type () {
      if (only_write) return kWrite;
      uint64_t rand_req_val = rand() % 100;
      if (kv_ssd_bench_type == 0)  {
          return (rand_req_val>=50)?kWrite:kRead;
      } else if (kv_ssd_bench_type == 1) {
          //return (rand_req_val>=95)?kWrite:kRead;
          return (rand_req_val>=trace_b_value)?kWrite:kRead;
      } else if (kv_ssd_bench_type == 2) { // YCSB-C
          return kRead;
      } else if (kv_ssd_bench_type == 3) { // YCSB-D, r:w = 95:5 read-latest
          return (rand_req_val>=95)?kWrite:kRead;
      } else if (kv_ssd_bench_type == 4) { // YCSB-E, s:w = 95:5
          //return kSeek;
          return (rand_req_val>=95)?kWrite:kSeek;
      } else if (kv_ssd_bench_type == 5) { // YCSB-F, r:u 50:50
          return (rand_req_val>=50)?kUpdate:kRead;
      } else if (kv_ssd_bench_type == 6) { // only write
          return kWrite;
      } else if (kv_ssd_bench_type == 7) { // only write
          return kRead;
      } else if (kv_ssd_bench_type == 8) { // only insert
          return kWrite;
      } else if (kv_ssd_bench_type == 9) { // only delete
          return (rand_req_val>=95)?kWrite:kDelete;
      } else if (kv_ssd_bench_type == 10) { // Mix Graph w/r/s=
          /* 
         int64_t rand_v;
  ...
  void Run() {
    ...
    du_enable_ = FLAGS_kv_ssd_du_enable;
    //kvssd_count_ = FLAGS_kv_ssd_count;
    printf("kvssd_count is set to %d %d\n", kvssd_count_, (int) FLAGS_kv_ssd_count);
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      //kvssd_compaction_enable_ = false;
      //kvssd_algo_value_ = 1;
      // Shard Size is too small....
      
      ChangeKeyRange();
      time_duration_ = 3000; // 10 min
      //time_duration_ = 1500; // 10 min
      kv_seq_opt_ = FLAGS_kv_seq_opt;
      if (FLAGS_kv_comp_opt == -1) {
        kv_comp_opt_ = 0; 
        kvssd_compaction_enable_ = false;
      } else {
        kv_comp_opt_ = FLAGS_kv_comp_opt; 
        kvssd_compaction_enable_ = true;
      }
      kv_ssd_enable_ = FLAGS_kv_ssd_enable;
      kvssd_algo_value_ = 2;
      du_kv_range_ = kv_ssd_key_range_;  
      //du_kv_range_ = FLAGS_kv_ssd_key_range;  
      process_fill = false;
      only_write = false;
      warmup_phase_ = 0;
      sleep(20);
      if (kv_ssd_enable_ == 3 || kv_ssd_enable_ == 4) {
          du_trivial_disable_ = true;
      } else {
          du_trivial_disable_ = false;
      }
      if (name == "fillseqdeterministic" ||
      ...
      } else if (name == Slice("trace_warm")) {
        kv_ssd_bench_type = 6;
        kv_ssd_pattern = 1;
        FLAGS_ycsb = true;
        time_duration_ = 1000; // 10 min
        //time_duration_ = 1500; // 10 min
        FLAGS_tail_latency = true;
        //LoadTrace();
        method = &Benchmark::RunTrace;
        ...
        FLAGS_ycsb = true;
        FLAGS_tail_latency = true;
        kv_ssd_threshold_ratio_ = 2;
        method = &Benchmark::RunTrace;
        time_duration_ = 1000; // 10 min
      } else if (name == Slice("trace_rw_1")) {
        kv_ssd_bench_type = 6;
        kv_ssd_pattern = 1;
        FLAGS_ycsb = true;
        FLAGS_tail_latency = true;
        kv_ssd_threshold_ratio_ = 1;
        method = &Benchmark::RunTrace;
      } else if (name == Slice("trace_rw_2")) {
        kv_ssd_bench_type = 6;
        kv_ssd_pattern = 1;
        FLAGS_ycsb = true;
        FLAGS_tail_latency = true;
        kv_ssd_threshold_ratio_ = 2;
        method = &Benchmark::RunTrace;
      } else if (name == Slice("trace_rw_3")) {
        kv_ssd_bench_type = 6;
        kv_ssd_pattern = 1;
        FLAGS_ycsb = true;
        FLAGS_tail_latency = true;
        kv_ssd_threshold_ratio_ = 3;
        method = &Benchmark::RunTrace;
      } else if (name == Slice("trace_sd")) {
        kv_ssd_bench_type = 9;
        kv_ssd_pattern = 0;
        FLAGS_ycsb = true;
        FLAGS_tail_latency = true;
        method = &Benchmark::RunTrace;
      } else if (name == Slice("trace_rd")) {
        kv_ssd_bench_type = 9;
        kv_ssd_pattern = 1;
        FLAGS_ycsb = true;
        FLAGS_tail_latency = true;
        ...
        char nvme_name[32];
        const char *  command;
        const char *  echo_command;
        for (int q = 0 ; q < 5 ; q ++) {
            std::string str = "nvme smart-log  ";
            std::string echo_str = "echo  ";
            if (q == 0) 
                strcpy(nvme_name, "/dev/nvme0n1");
            else if (q == 1) 
                strcpy(nvme_name, "/dev/nvme1n1");
            else if (q == 2) 
                strcpy(nvme_name, "/dev/nvme2n1");
            else if (q == 3) 
                strcpy(nvme_name, "/dev/nvme3n1");
            else if (q == 4) 
                strcpy(nvme_name, "/dev/nvme4n1");
            str = str + nvme_name ;
            echo_str = echo_str + nvme_name + " " + "Trace Fill" ;
            command = str.c_str();
            echo_command = echo_str.c_str();
            ret = system(echo_command); 
            //printf("Start of Benchmark Smart Log of NVMe %s cmd %s \n", nvme_name, command);
            ret = system(command);
        ...
        */
        ...
        for (int q = 0 ; q < 5 ; q ++) {
            ...
            if (q == 0) 
                ...
                strcpy(nvme_name, "/dev/nvme4n1");
            str = str + nvme_name ;
            echo_str = echo_str + nvme_name + " " + "Trace Stats" ;
            command = str.c_str();
            echo_command = echo_str.c_str();
            ret = system(echo_command); 
            //printf("Start of Benchmark Smart Log of NVMe %s cmd %s \n", nvme_name, command);
      ...
      if (method != nullptr) {
        ...
        kv_ssd_thread_gap_ = kv_ssd_key_range_ / num_threads;
        CombinedStats combined_stats;
        for (int i = 0; i < num_repeat; i++) {
          Stats stats = RunBenchmark(num_threads, name, method);
          printf("RunBenchmark done here * ===================\n");
          printf("RunBenchmark done here * ===================\n");
          printf("RunBenchmark done here * ===================\n");
          printf("RunBenchmark done here * ===================\n");
      ...
      if (kv_ssd_value_list_ != nullptr) {
        free (kv_ssd_value_list_);
      }
 ...
 private:
  ...
  Stats RunBenchmark(int n, Slice name,
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (name == Slice("trace_fill")) { // TraceFill 
          uint64_t gap = kv_ssd_key_range_ / n; 
          //uint64_t gap = FLAGS_kv_ssd_key_range / n; 
          arg[i].thread->start_key_num = (gap * i) + 1;
          printf("Trace Fill thread %d start key %lld gap %lld\n", i, (long long int)arg[i].thread->start_key_num, (long long int)gap);
      }
      else {
          //arg[i].thread->start_key_num = 1; 
          arg[i].thread->start_key_num = (rand() % kv_ssd_key_range_) + 1; 
          //arg[i].thread->start_key_num = (rand() % FLAGS_kv_ssd_key_range) + 1; 
          printf(" thread %d start key %lld \n", i, (long long int)arg[i].thread->start_key_num);
      }
  ...
  void DoWrite(ThreadState* thread, WriteMode write_mode) {
    int test_duration = write_mode == RANDOM ? FLAGS_duration : 0;
    const int64_t num_ops = writes_ == 0 ? num_ : writes_;

    if (test_fillseq == 1) {
        test_duration = 0 ; 
    } else if (test_overwrite == 1) {
        test_duration = 1500; 
    }
...
// optimization, so skip the check of the max level.
  ...
  int64_t ParetoCdfInversion(double u, double theta1, double k, double sigma) {
    double ret;
    if (k == 0.0) {
      ret = theta1 - sigma * std::log(u);
    } else {
      ret = theta1 + sigma * (std::pow(u, -1 * k) - 1) / k;
    }
...
/*
   void MixGraphDU(ThreadState* thread) {
    int64_t gets = 0;
    int64_t puts = 0;
    int64_t get_found = 0;
    int64_t seek = 0;
    ...
    DB* db = SelectDB(thread);
    //Duration duration(FLAGS_duration, reads_);
    Duration duration(time_duration_, 0); 
    while (!duration.Done(1)) {
      ...
      // Create by DU Key
      snprintf(key, sizeof(key), "%016lu", key_rand);
      //GenerateKeyFromInt(key_rand, FLAGS_num, &key);
      int query_type = query.GetType(rand_v);
      ...
      if (query_type == 0) {
        ...
        // Seek query
          ...
          int64_t bytes = 0;
          iter->Seek(key);
          for (; qq < scan_length && iter->Valid(); iter->Next()) {
              bytes += iter->key().size() + iter->value().size();
              thread->stats.FinishedOps(nullptr, db, 1, kRead);
              ++qq;
          }
  ...
  void DoDelete(ThreadState* thread, bool seq) {
    ...
    while (!duration.Done(entries_per_batch_)) {
      ...
      for (int64_t j = 0; j < entries_per_batch_; ++j) {
        ...
        // TODO DU Change Here
        snprintf(key, sizeof(key), "%016lu", k);
  ...
  bool read_intensive_ = false;
  long long int write_op_ = 0; 
  long long int read_op_ = 0; 
  void is_read_intensive(int op, int thread_id) {
      if (thread_id == 0) {
          if (op == 0) {
              write_op_ ++;
          } else if (op == 1) {
      ...
      long long int total = read_op_ + write_op_;
      if (total > 2000) {
          float wr_ratio = 100 * write_op_ / total; 
          float rd_ratio = 100 * read_op_ / total; 
          if (wr_ratio < 10) {
              read_intensive_ = true;
          }
          if (rand() % 1000 == 0) {
            printf("%s wr_ratio %f rd_ratio %f\n", __func__, wr_ratio, rd_ratio);
          }
          read_op_ = 0;
          write_op_ = 0;
      }
  ...
  long long int scan_bytes = 0;
  GenerateTwoTermExpKeys du_gen_exp[32];
  QueryDecider du_query[32]; // = db->NewIterator(options);
  Iterator* du_iter[32]; // = db->NewIterator(options);
  KVIter du_kviter[32]; // = db->NewIterator(options);
  uint64_t qqqqq_cnt = 0;
  char my_kv_value[65536];
  char value_ppp[32][65536];
  void RunOpNew(ThreadState* thread, DB * db, RandomGenerator * gen) {
      int scan_length = 100;
      bool l0_hit = false;
      bool last_hit = false;
      int migration = 0;
      ...
      read_operations.kv_ssd_id = thread->tid;
      read_operations.l0_hit = &l0_hit;
      read_operations.last_hit = &last_hit;
      read_operations.migration = &migration;
      int new_value_size = FLAGS_value_size; 
      Status s;
      char key[100];
      char key_1[100];
      uint64_t key_num = 0;
      if (kv_ssd_pattern == 0) { // seq
          key_num = get_seq_key(thread); 
      } else if (kv_ssd_pattern == 1) { // rand
          key_num = key_seq[rand() % kv_ssd_key_range_]; 
          //key_num = key_seq[rand() % FLAGS_kv_ssd_key_range]; 
      } else if (kv_ssd_pattern == 2) { // zipf
          key_num = get_key(thread->tid); 
      } else if (kv_ssd_pattern == 3) { // Mix
          //int query_type = query.GetType(rand_v);
      } else if (kv_ssd_pattern == 4) {
          key_num = get_key(thread->tid);
          /*
          bool success = true;
          key_num = get_latest(&success);
          if (success == false) {
              key_num = 
          }
          */
      }
      if (FLAGS_kv_ssd_value_change)
      {
#if (1) 
          uint64_t rand_value = 0;
          if (kv_ssd_enable_ == 2) {
              if (kv_ssd_bench_type == 8) {
                  rand_value = (uint64_t) key_num % freq_sum;
              } else {
                  rand_value = (uint64_t) (key_num + ((int) freq_sum / 2)) % freq_sum;
              }
          } else {
              rand_value = (uint64_t) rand() % freq_sum;
          }
          if (FLAGS_kv_ssd_distribution <= 1) { // Same Frequency // Same Capacity (Propotional)
              if (FLAGS_kv_ssd_distribution <= 1) { // Same Frequency // Same Capacity (Propotional)
                  for (int k = 0 ; k < FLAGS_kv_ssd_value_count ; k ++) {
                      if ((int)(rand_value % freq_sum) < freq_list[k]) {
                          //db_size_sum += value_list[k];
                          new_value_size = value_list[k];
                          break;
                      }
                  }
              } 
          } else if (FLAGS_kv_ssd_distribution == 2) { // Same Capacity with successive value sizes
          } else if (FLAGS_kv_ssd_distribution == 3) { // Exponential (MixGraph)
              int64_t ini_rand = rand();
              int64_t rand_v = (ini_rand % 367001606) + 1;
              double u = static_cast<double>(rand_v) / 367001607;
              int64_t pareto_val = 20 * ParetoCdfInversion(u, FLAGS_value_theta, FLAGS_value_k, FLAGS_value_sigma);
              if (FLAGS_kv_ssd_mix_min_value > pareto_val) pareto_val = FLAGS_kv_ssd_mix_min_value;
              if (FLAGS_kv_ssd_mix_max_value < pareto_val) pareto_val = FLAGS_kv_ssd_mix_max_value;
              new_value_size = pareto_val;
              if (rand() % 100000 == 0) 
                  printf(" %d\n", (int)pareto_val);
          } else if (FLAGS_kv_ssd_distribution == 4) {
              new_value_size = GetSame(); 
          } else if (FLAGS_kv_ssd_distribution == 5) {
              new_value_size = GetPareto(); 
          }
#else
#endif
      } else {
          uint64_t rand_value = (uint64_t) key_num;
          // Value size is never Changed Always Update Same value size 
          if (FLAGS_kv_ssd_distribution <= 1) { // Same Frequency // Same Capacity (Propotional)
              if (FLAGS_kv_ssd_distribution <= 1) { // Same Frequency // Same Capacity (Propotional)
                  for (int k = 0 ; k < FLAGS_kv_ssd_value_count ; k ++) {
                      if ((int)(rand_value % freq_sum) < freq_list[k]) {
      ...
      snprintf(key, sizeof(key), "%016lu", key_num);
      OperationType op = get_req_type();
   
      if (op == kWrite) {
          is_read_intensive(0, thread->tid);
          // Only for Analysis 
          if (process_fill) {
              total_rocksdb_size_ += new_value_size; 
          }
          if (kv_ssd_pattern == 4) {
              put_latest(key_num);
          }

          if (kv_ssd_enable_ == 1) { // Only KV SSD
              if (kvssd_count_ == 1) {
                  kv_ssd_->kv_put_buffer_sync(key, 16, (char *) gen->Generate(new_value_size).data(), new_value_size, thread->tid % 64);
              } else {
                  if (key_num % 2 == 0) {
                    kv_ssd_->kv_put_buffer_sync(key, 16, (char *) gen->Generate(new_value_size).data(), new_value_size, thread->tid % 64);
                  } else {
                    kv_ssd_new_->kv_put_buffer_sync(key, 16, (char *) gen->Generate(new_value_size).data(), new_value_size, thread->tid % 64);
                  }
              }
          } else if (kv_ssd_enable_ == 2 && new_value_size >= 4096) {
              kv_ssd_->kv_put_buffer_sync(key, 16, (char *) gen->Generate(new_value_size).data(), new_value_size, thread->tid % 64);
          } else {
              s = db->Put(write_options_, key, gen->Generate(new_value_size));
          }
          ...
          is_read_intensive(1, thread->tid);
          if (kv_ssd_pattern == 4) {
              if (key_latest.size() != 0) { 
                  key_num = get_latest();
                  snprintf(key, sizeof(key), "%016lu", key_num);
              }
          }
          std::string value;
          // TODO Removable Part.. it will be removed in the future 
          if (kv_ssd_enable_ == 1) {
              if (kvssd_count_ == 1) {
                  char value_p[16384]; 
                  kv_ssd_->kv_get_buffer_sync(key, 16, value_p, 16384, 0);
              } else {
                  char value_p[16384]; 
                  if (key_num % 2 == 0) {
                      kv_ssd_->kv_get_buffer_sync(key, 16, value_p, 16384, 0);
                  } else {
                      kv_ssd_new_->kv_get_buffer_sync(key, 16, value_p, 16384, 0);
                  }
              }
              thread->stats.FinishedOps(nullptr, db, 1, kRead);
          }
              ...
              kv_ssd_->kv_get_buffer_sync(key, 16, value_p, 16384, 0);
              qqqqq_cnt++;
              if (qqqqq_cnt % 10000 == 0) printf("It is work ! QQQQ cnt %lld\n", (long long int) qqqqq_cnt);
              thread->stats.FinishedOps(nullptr, db, 1, kRead);
          } else {
              s = db->Get(read_operations, key, &value);
              //printf("read options...%p %p %p\n", read_operations.l0_hit, read_operations.last_hit, read_operations.migration);
              if (l0_hit) {
#if (KV_PREFETCH == 1)
                  if (!(kv_ssd_pattern == 4 || kv_ssd_bench_type == 4)) {
                      int prefetch_num = kv_index_->getPrefetchNum(thread->tid);
                      char kv_value[4][FLAGS_kv_ssd_max_value];
                      for (int kk = 0 ; kk < prefetch_num ; kk ++) {
                          PreE * element = kv_index_->getPrefetchBuffer(thread->tid, nullptr, key, nullptr);
                          if (element) {
                              while (element->status_ == 2) {}
                              element->status_ = 2;
                              kv_ssd_->kv_get_buffer_async((const char *)element->key_, 16, kv_value[kk], 16384, &element->status_); 
                          }
                      }
                  }
#endif
                  ...
                  // if (!(trace_d || trace_d)) 
                  if (!(kv_ssd_pattern == 4 || kv_ssd_bench_type == 4)) {
                      int prefetch_num = kv_comp_->getPrefetchNum(thread->tid);
                      char kv_value[4][FLAGS_kv_ssd_max_value];
                      for (int kk = 0 ; kk < prefetch_num ; kk ++) {
                          PreE * element = kv_comp_->getPrefetchBuffer(thread->tid, nullptr, key, nullptr);
                          if (element) {
                              while (element->status_ == 2) {}
                              element->status_ = 2;
                              kv_ssd_->kv_get_buffer_async((const char *)element->key_, 16, kv_value[kk], 16384, &element->status_); 
              ...
              if (kv_ssd_enable_ == 4) {
                  if (rand() % 1000 == 0) {
                      printf("Mig Go %d No %d\n", mig_go_, mig_no_);
                  }
                  if (migration != 0 && read_intensive_ == true) {  
                      //if (migration && read_intensive_ == true) {  
                      if (migration == 1) {
                          WriteOptions migration_option;
                          memcpy((void *) &migration_option, (void *)&write_options_, sizeof(WriteOptions));
                          migration_option.direction = 1;
                          kv_worker_->kvEnqueue(key, 16, 0, (char *) gen->Generate(new_value_size).data(), new_value_size, migration_option.direction);
                          //s = db->Put(migration_option, key, gen->Generate(new_value_size));
                          thread->stats.FinishedOps(nullptr, db, 1, kWrite);
                          mig_go_ += 1;
                      } else if (migration == 2) {
                          WriteOptions migration_option;
                          memcpy((void *) &migration_option, (void *)&write_options_, sizeof(WriteOptions));
                          migration_option.direction = 2;
                          s = db->Put(migration_option, key, gen->Generate(new_value_size));
                          thread->stats.FinishedOps(nullptr, db, 1, kWrite);
                          mig_go_ += 1;
                      }
          ...
          */
          if (kv_ssd_enable_ == 1) {
              if (kvssd_count_ == 1) {
                  ...
                  char value_p[16384]; 
                  for (; i < scan_length; i ++) {
                      snprintf(key, sizeof(key), "%016lu", ((key_num + i) % kv_ssd_key_range_) + 1);
                      //snprintf(key, sizeof(key), "%016lu", ((key_num + i) % FLAGS_kv_ssd_key_range) + 1);
                      kv_ssd_->kv_get_buffer_sync(key, 16, value_p, 16384, 0);
                      thread->stats.FinishedOps(nullptr, db, 1, kRead);
                  }
              } else {
                  int64_t i = 0 ;
                  char value_p[FLAGS_kv_ssd_max_value];
                  volatile int status[128] = {0};
                  volatile int status_index = 0;
                  for (; i < scan_length / 2 ; i ++) {
                      //snprintf(key, sizeof(key), "%016lu", ((key_num + i*2) % FLAGS_kv_ssd_key_range) + 1);
                      //snprintf(key_1, sizeof(key_1), "%016lu", ((key_num + i*2 + 1) % FLAGS_kv_ssd_key_range) + 1);
                      snprintf(key, sizeof(key), "%016lu", ((key_num + i*2) % kv_ssd_key_range_) + 1);
                      snprintf(key_1, sizeof(key_1), "%016lu", ((key_num + i*2 + 1) % kv_ssd_key_range_) + 1);
                      while (status[status_index] == 2) {}
                      status[status_index] = 2;
                      kv_ssd_->kv_get_buffer_async(key, 16, value_p, 16384, &status[status_index]);
                      status_index = (status_index + 1) % 128;
                      //thread->stats.FinishedOps(nullptr, db, 1, kRead);

                      //printf("Index %d Start 22 stauts_index %d %d \n", (int)i, status_index, status[status_index]);
                      while (status[status_index] == 2) {}
                      status[status_index] = 2;
                      status[status_index] = 0;
                      //kv_ssd_new_->kv_get_buffer_async(key_1, 16, value_p, 16384, &status[status_index]);
                      status_index = (status_index + 1) % 128;
                      //thread->stats.FinishedOps(nullptr, db, 1, kRead);
                      //printf("Index %d End index%d \n", (int) i, status_index);
                  }
                  for (i = 0 ; i < 128 ; i ++) {
                      while (status[i] == 2) {}
                  }
              ...
              int i = 0;
              Iterator* scan_it = db->NewIterator(read_operations);
              for (scan_it->Seek(key); scan_it->Valid() && i < scan_length; scan_it->Next()) {
                  i++;
#if (1) 
                  // Copy out iterator's value to make sure we read them.
                  Slice copy_value = scan_it->value();
                  memcpy(value_ppp, copy_value.data(), 
                          std::min(copy_value.size(), (long unsigned int) 16384));
                  scan_bytes += (long long int) scan_it->key().size() + scan_it->value().size();
#endif
              }
              thread->stats.FinishedOps(nullptr, db, 1, kSeek);
              if (rand() % 1000 == 0) printf("scan bytes %lld\n", scan_bytes);
              delete scan_it;
 
              if (kv_ssd_enable_ == 3 || kv_ssd_enable_ == 4) {
                  bool iter_ret = kv_index_->makeIter(&du_kviter[thread->tid], key, scan_length, 0, thread->tid);
                  if (iter_ret) {
                      bool target_last = false;
                      char target_key[17];
                      size_t target_key_size = 0;
                      volatile int qq_status[64] = {0};
                      volatile int async_index = 0;
                      int async_count = 0;

                      while (target_last != true) {
                          kv_index_->nextIter(&du_kviter[thread->tid], target_key, &target_key_size, &target_last);
                          if (target_key_size == 0) continue;
                          while (qq_status[async_index] == 2) {}
                          qq_status[async_index] = 2;
                          kv_ssd_->kv_get_buffer_async(key, 16, value_ppp[thread->tid], 16384, &qq_status[async_index]); 
                          async_index = (async_index + 1) % 64; 
                          async_count ++;
                      }
                      for (int qq = 0 ; qq < 64 ; qq ++) {
                          while (qq_status[qq] == 2) {}
                      }
                      
                      printf("async count %d\n", async_count);
                      fflush(stdout);
                      kv_index_->releaseIter(&du_kviter[thread->tid], thread->tid);
                  } else {
                    printf("Not Found Here !!!!!!!!!!!!! It will cause error?\n"); 
                  }
                  /*
                  iter_ret = kv_comp_->makeIter(&du_kviter[thread->tid], key, scan_length, 0, thread->tid);
                  if (iter_ret) {
                      bool target_last = false;
                      char target_key[17];
                      size_t target_key_size = 0;
                      ...
                      while (target_last != true) {
                          kv_comp_->nextIter(&du_kviter[thread->tid], target_key, &target_key_size, &target_last);
                          if (target_key_size == 0) continue;
                          while (qq_status[async_index] == 2) {}
                          qq_status[async_index] = 2;
                          kv_ssd_->kv_get_buffer_async(key, 16, value_ppp[thread->tid], 16384, &qq_status[async_index]); 
                      ...
                      kv_comp_->releaseIter(&du_kviter[thread->tid], thread->tid);
                  }
                  */
              } else {
              }
          ...
          if (kv_ssd_enable_ == 1) {
              if (kvssd_count_ == 1) {
                  char value_p[16384]; 
                  kv_ssd_->kv_get_buffer_sync(key, 16, value_p, 16384, 0);
                  kv_ssd_->kv_put_buffer_sync(key, 16, (char *) value_p, new_value_size, thread->tid % 64);
              } else {
                  char value_p[16384]; 
                  if (key_num % 2 == 0) {
                      kv_ssd_->kv_get_buffer_sync(key, 16, value_p, 16384, 0);
                      ...
                      kv_ssd_new_->kv_get_buffer_sync(key, 16, value_p, 16384, 0);
                      kv_ssd_new_->kv_put_buffer_sync(key, 16, (char *) value_p, new_value_size, thread->tid % 64);
                  }
              }
          } else { 
              std::string value;
              if (kv_ssd_enable_ == 2 && new_value_size >= 4096) {
                  char value_p[16384]; 
                  kv_ssd_->kv_get_buffer_sync(key, 16, value_p, 16384, 0);
                  qqqqq_cnt++;
                  if (qqqqq_cnt % 10000 == 0) printf("It is work ! QQQQ cnt %lld\n", (long long int) qqqqq_cnt);
  ...
  void RunTestVersion(ThreadState* thread, DB * db, RandomGenerator * gen) {
      //RandomGenerator gen;
      ReadOptions read_operations;
      int new_value_size = FLAGS_value_size; 
      Status s;
      ...
      uint64_t target_1 = (uint64_t)FLAGS_kv_ssd_value_small ;
      uint64_t target_2 = (uint64_t)FLAGS_kv_ssd_value_small + (uint64_t)FLAGS_kv_ssd_value_medium ;
      uint64_t target_3 = (uint64_t)FLAGS_kv_ssd_value_small + (uint64_t)FLAGS_kv_ssd_value_medium + (uint64_t)FLAGS_kv_ssd_value_large;
      if (FLAGS_kv_ssd_value_change)
      {
          // 1 2 4 => 7 
          if ((key_num % target_3) < target_1) {
              new_value_size = 256;
          } else if ((key_num % target_3) < target_2) {
              new_value_size = 1024;
          } else if ((key_num % target_3) < target_3) {
              new_value_size = 4096;
          }
      }
      ...
      if (op == kWrite) {
          if (kv_ssd_enable_ == 1) { // Only KV SSD
              kv_ssd_->kv_put_buffer_sync(key, 16, gen->Generate(new_value_size).data(), new_value_size, thread->tid % 64);
              s = Status::OK(); 
          } else if (kv_ssd_enable_ == 2) { // Mixed 
              if (new_value_size >= 4096) {
                  kv_ssd_->kv_put_buffer_sync(key, 16, gen->Generate(new_value_size).data(), new_value_size, thread->tid % 64);
                  s = Status::OK(); 
              } else {
                  s = db->Put(write_options_, key, gen->Generate(new_value_size));
          ...
          std::string value;
          if (kv_ssd_enable_ == 1) { // Only KV
              char kv_value[FLAGS_kv_ssd_max_value];
              kv_ssd_->kv_get_buffer_sync(key, 16, kv_value, FLAGS_kv_ssd_max_value, thread->tid % 64);
          } else if (kv_ssd_enable_ == 2) { // Mixed 
              // TODO It is unfair! 
              if (new_value_size >= 4096) {
                  char kv_value[FLAGS_kv_ssd_max_value];
  ...
  void RunTrace(ThreadState* thread) {
    ...
    ReadOptions read_operations;
    Duration duration(time_duration_, 0); 
    //Duration duration(6000, 0); 
    thread->stats.ResetLastOpTime();
    DB* db = SelectDB(thread);
  
    // FLAGS_kv_ssd_max_value
    printf("This is point max value %lld\n", (long long int) FLAGS_kv_ssd_max_value);
    //exit(-1);
#if (1)
    uint64_t key_done = 0;
    float prev_progress = 0;
    int warmup_phase = warmup_phase_;
    du_gen_exp[thread->tid].InitiateExpDistribution(
          kv_ssd_key_range_, FLAGS_keyrange_dist_a, FLAGS_keyrange_dist_b,
          //FLAGS_kv_ssd_key_range, FLAGS_keyrange_dist_a, FLAGS_keyrange_dist_b,
          FLAGS_keyrange_dist_c, FLAGS_keyrange_dist_d);
    std::vector<double> ratio{FLAGS_mix_get_ratio, FLAGS_mix_put_ratio,
                              FLAGS_mix_seek_ratio};
    du_query[thread->tid].Initiate(ratio);
    //du_iter[thread->tid] = db->NewIterator(read_operations);
/*
    if (kv_ssd_bench_type == 10 || kv_ssd_bench_type == 4) {
        kv_index_->allocIter(&(du_kviter[thread->tid])); 
    //    kv_comp_->allocIter(&(du_kviter[thread->tid])); 
    }
*/
    while (1) {
        RunOpNew(thread, db, &gen);

        //RunTestVersion(thread, db, &gen);
        if (warmup_phase != 0) {
            if (duration.Done(1, key_done, thread->tid)) {
                printf("Warmup is Done !\n");
                break;
            }
            key_done ++;
        } else if (kv_ssd_bench_type != 8) {
            if (duration.Done(1, key_done, thread->tid)) break;
            key_done ++;
        } else {
            // Multi threaded Trace fill
            if (kv_ssd_enable_ == 1) {
                uint64_t key_done_check = (kv_ssd_key_range_ / (kv_ssd_threads - 1));
                //uint64_t key_done_check = (FLAGS_kv_ssd_key_range / (kv_ssd_threads - 1));
                //if (true) {
                if (key_done == key_done_check) {
                    printf("Trace Fill Done here ! range %lld thread %d -> %d check %lld\n", (long long int) kv_ssd_key_range_, kv_ssd_threads, kv_ssd_threads-1, (long long int) key_done_check);
                    //printf("Trace Fill Done here ! range %lld thread %d -> %d check %lld\n", (long long int) FLAGS_kv_ssd_key_range, kv_ssd_threads, kv_ssd_threads-1, (long long int) key_done_check);
                    break;
                }
                key_done ++;
            }
            // Single threaded Trace fill
            else {
                if (key_done == kv_ssd_key_range_) {
                //if (key_done == FLAGS_kv_ssd_key_range) {
                    if (kv_ssd_enable_ == 3 || kv_ssd_enable_ == 4) {
                        kv_ssd_->kvFinishFill();
                    }
                    printf("\t\t =============== ********** Trace Fill Done here !\n");
                    break;
                }
                key_done ++;
            }
            float progress = 100 * ((float) key_done / kv_ssd_key_range_);
            //float progress = 100 * ((float) key_done / FLAGS_kv_ssd_key_range);
            if (rand() % 1000 == 0) {
            //if ((progress - prev_progress) > 10) {
                printf("Current Progress is %f prev %f %lld %lld \n", progress, prev_progress, (long long int) key_done, (long long int) kv_ssd_key_range_);
                //printf("Current Progress is %f prev %f %lld %lld \n", progress, prev_progress, (long long int) key_done, (long long int) FLAGS_kv_ssd_key_range);
...
/*
    if (kv_ssd_bench_type == 10 || kv_ssd_bench_type == 4) {
        kv_index_->freeIter(&(du_kviter[thread->tid])); 
    //    kv_comp_->freeIter(&(du_kviter[thread->tid]));
    }
*/
#else
    long key_done = 0;
  ...
  /*
     FLAGS_kv_ssd_db_size
     FLAGS_kv_ssd_value_count
     FLAGS_kv_ssd_first_value
  */

void ParetoLocal (long long int db_size) {
    // Million
    long long int target_db_size = db_size * 1024 * 1024 * 1024;
    long long int total_key_count = 1000000;
    long long int median_key = total_key_count / 2;
    double sigma = FLAGS_value_sigma * 30;
    printf("%s PARRRRRRRRRREEEEEEEEETTTTTTTTTTTOOOOOOOOO %lf value sigma %lf\n", __func__, sigma, FLAGS_value_sigma);
    //double sigma = 25.45;
    double k = FLAGS_value_k;
    long long int sum_of_value = 0;
    long long int large_value_sum = 0;
    long long int small_value_sum = 0;
    for (int i = 1 ; i < total_key_count ; i ++) {
        double u = static_cast<double>(i) / total_key_count;
        long long int my_value = sigma * (std::pow(u, -1 * k) - 1) / k; 
        if (my_value < FLAGS_kv_ssd_mix_min_value) my_value = FLAGS_kv_ssd_mix_min_value;
        if (my_value > FLAGS_kv_ssd_mix_max_value) my_value = FLAGS_kv_ssd_mix_max_value;
        //if (my_value < 256) my_value = 256;
        //if (my_value > 8192) my_value = 8192;
        sum_of_value += my_value;
        //if (i % 1000 == 0) 
        //    printf ("%lld\n", my_value);
        
        if (my_value >= 4096) {
            large_value_sum += my_value;
        }
        if (my_value <= 256) {
            small_value_sum += my_value;
        }
    }
     
    double u = static_cast<double>(median_key) / total_key_count;
    long long int median = sigma * (std::pow(u, -1 * k) - 1) / k;
    printf("sum %lld large %lld small %lld median %lld \n", sum_of_value, large_value_sum, small_value_sum, median);
  
    int repeat_cnt = target_db_size / sum_of_value;
    printf("%lld %lld repeat %d\n", target_db_size, sum_of_value, repeat_cnt);
    /*
    sum_of_value = 0;
    for (long long int i = 0 ; i < total_key_count * repeat_cnt ; i ++) {
        double u = static_cast<double>(rand() % total_key_count) / total_key_count;
        long long int my_value = sigma * (std::pow(u, -1 * k) - 1) / k; 
        if (my_value < 256) my_value = 256;
        if (my_value > 16384) my_value = 16384;
        sum_of_value += my_value;
        
    }
    printf("total sum %lld\n", sum_of_value/1024/1024);
    */
    kv_ssd_key_range_ = total_key_count * repeat_cnt;
    kv_ssd_qwer_cnt_ = total_key_count;
}

void Same(long long int db_size) {
    printf("Start Same\n");
    fflush(stdout);
    // Millio
    long long int target_db_size = db_size * 1024 * 1024 * 1024;
   
    long long int key_range = 1;
    long long int current_value = FLAGS_kv_ssd_mix_min_value; 
    long long int threshold = 2 * FLAGS_kv_ssd_mix_min_value; 
    long long int sum_of_value = 0;
    int index = 1;

    kv_ssd_value_list_ = (int *) malloc(sizeof(int)*FLAGS_kv_ssd_mix_max_value);
    while (1) {
        sum_of_value += current_value;
        kv_ssd_value_list_[key_range-1] = current_value;
        ...
        if (current_value >= FLAGS_kv_ssd_mix_max_value) break;
        key_range ++;
    }

    int repeat_cnt = target_db_size / sum_of_value;
    printf("repeat %d min value %lld max value %lld last_value %lld\n", repeat_cnt, (long long int) FLAGS_kv_ssd_mix_min_value,(long long int) FLAGS_kv_ssd_mix_min_value,(long long int) current_value);
    printf("My Key Range %lld \n", key_range * repeat_cnt);
    /*
    sum_of_value = 0;
    for (long long int i = 0 ; i < key_range * repeat_cnt ; i ++) {
        int index = rand() % key_range;
        //printf("Index %d range %d\n", index, key_range);
        int my_value = kv_ssd_value_list_[index];
        sum_of_value += my_value;
    }
    printf("total sum %lld\n", sum_of_value/1024/1024);
    */

    kv_ssd_key_range_ = key_range * repeat_cnt;
    kv_ssd_qwer_cnt_ = key_range;
    printf("kv_ssd_key_range %lld %lld Done Same\n", (long long int) kv_ssd_key_range_, (long long int) kv_ssd_qwer_cnt_);
    fflush(stdout);
}
  ...
  int GetPareto() {
      double sigma = FLAGS_value_sigma * 30;
      double k = FLAGS_value_k;
      double u = static_cast<double>(rand() % kv_ssd_qwer_cnt_) / kv_ssd_qwer_cnt_;
      long long int my_value = sigma * (std::pow(u, -1 * k) - 1) / k; 
      if (my_value < FLAGS_kv_ssd_mix_min_value) my_value = FLAGS_kv_ssd_mix_min_value;
      if (my_value > FLAGS_kv_ssd_mix_max_value) my_value = FLAGS_kv_ssd_mix_max_value;
      return (int)my_value;
  ...
  int GetSame() {
        int index = rand() % kv_ssd_qwer_cnt_;
        int my_value = kv_ssd_value_list_[index];
        //sum_of_value += my_value;
        
        if (my_value < FLAGS_kv_ssd_mix_min_value) {
            printf("%s Error case index %d value %d\n", __func__, index, my_value);
            my_value = FLAGS_kv_ssd_mix_min_value;
        }
        if (my_value > FLAGS_kv_ssd_mix_max_value) {
            printf("%s Error case index %d value %d\n", __func__, index, my_value);
            my_value = FLAGS_kv_ssd_mix_max_value;
        }
        return my_value;
  }
  ...
  void ChangeKeyRange() {
      ...
      uint64_t target_db_size = FLAGS_kv_ssd_db_size * 1024 * 1024 * 1024;
      uint64_t db_size_sum = 0;
      uint64_t index = 0;
      int64_t first_value = FLAGS_kv_ssd_first_value;
      //int * value_list = (int *) malloc(sizeof(int) * FLAGS_kv_ssd_value_count);
      value_list = (int *) malloc(sizeof(int) * FLAGS_kv_ssd_value_count);
      value_list[0] = first_value;
      freq_list = (int *) malloc(sizeof(int) * FLAGS_kv_ssd_value_count);
      //int * freq_list = (int *) malloc(sizeof(int) * FLAGS_kv_ssd_value_count);
      for (int i = 1 ; i < FLAGS_kv_ssd_value_count ; i ++) {
          value_list[i] = value_list[i-1] * FLAGS_kv_ssd_value_ratio;
      }

      for (int i = 0 ; i < FLAGS_kv_ssd_value_count ; i ++) {
          if (FLAGS_kv_ssd_distribution == 0) { // Same Frequency
              freq_list[i] = 1;
          } else if (FLAGS_kv_ssd_distribution == 1) { // Same Capacity (Propotional)
              if ( i == 0) {
                  freq_list[FLAGS_kv_ssd_value_count - 1] = 1; 
              } else { 
                  freq_list[FLAGS_kv_ssd_value_count - i - 1] = freq_list[FLAGS_kv_ssd_value_count - i] * FLAGS_kv_ssd_value_ratio; 
              }
          } else if (FLAGS_kv_ssd_distribution == 2) { // Same Capacity with successive value sizes
            // Not here
          } else if (FLAGS_kv_ssd_distribution == 3) { // Exponential (MixGraph)
            // Not here
          } else if (FLAGS_kv_ssd_distribution == 4) { // Exponential (MixGraph)
            break;
          } else if (FLAGS_kv_ssd_distribution == 5) { // Exponential (MixGraph)
            break;
          }
      }
      freq_sum = 0;
      ...
      for (int i = 0 ; i < FLAGS_kv_ssd_value_count ; i++) {
          ...
          if (i != 0) {
              freq_list[i] = freq_list[i-1] + freq_list[i];
          }
      }
      while (1) {
          if (FLAGS_kv_ssd_distribution <= 1) { // Same Frequency // Same Capacity (Propotional)
              for (int k = 0 ; k < FLAGS_kv_ssd_value_count ; k ++) {
                  if ((int)(index % freq_sum) < freq_list[k]){
                      db_size_sum += value_list[k];
                      break;
                  }
              }
          } else if (FLAGS_kv_ssd_distribution == 2) { // Same Capacity with successive value sizes
              //db_size_sum += get_value_size();
          } else if (FLAGS_kv_ssd_distribution == 3) { // Exponential (MixGraph)
              int64_t ini_rand = rand();
              int64_t rand_v = (ini_rand % 367001606) + 1;
              double u = static_cast<double>(rand_v) / 367001607;
              int64_t pareto_val = 20 * ParetoCdfInversion(u, FLAGS_value_theta, FLAGS_value_k, FLAGS_value_sigma);
              //printf("Pareto val prev %d\t", (int)pareto_val);
              if (FLAGS_kv_ssd_mix_min_value > pareto_val) pareto_val = FLAGS_kv_ssd_mix_min_value;
              if (FLAGS_kv_ssd_mix_max_value < pareto_val) pareto_val = FLAGS_kv_ssd_mix_max_value;
              db_size_sum += pareto_val;
              if (rand() % 10000 == 0) 
                  printf(" %d\n", (int)pareto_val);
          
          } else if (FLAGS_kv_ssd_distribution == 4) { // Same Capacity with successive value sizes
              break;
          } else if (FLAGS_kv_ssd_distribution == 5) { // Same Capacity with successive value sizes
              break;
          } 
          
          if (db_size_sum > target_db_size) {
              printf("target db_size %lld now db size %lld\n", (long long int) target_db_size, (long long int) db_size_sum);
              break;
          }
          index ++;
      }
      // index will be key range


      printf("Key Range is %lld inpute key range %lld\n", (long long int) index, (long long int) FLAGS_kv_ssd_key_range);
   
      printf("Freq Sum %ld\n", (long int) freq_sum);
      for (int i = 0 ; i < FLAGS_kv_ssd_value_count ; i ++) {
          printf("Index %6d : Value %lld : Freq %lld\n", i, (long long int) value_list[i], (long long int) freq_list[i]);
      }

      kv_ssd_qwer_cnt_ = 0;
      kv_ssd_key_range_ = index;


      if (FLAGS_kv_ssd_distribution == 4) {
        Same(FLAGS_kv_ssd_db_size);
        printf("Fucking Done here\n");
        fflush(stdout);
      } else if (FLAGS_kv_ssd_distribution == 5) {
        ParetoLocal(FLAGS_kv_ssd_db_size);
      } 
      
      printf("QQQQ Key Range is %lld inpute key range %lld\n", (long long int) index, (long long int) FLAGS_kv_ssd_key_range);
      fflush(stdout);
  }
