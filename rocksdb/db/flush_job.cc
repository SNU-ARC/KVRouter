...
int flush_cnt_;
std::vector<uint64_t> flush_print_;
std::vector<uint64_t> flush_index_;
std::vector<uint64_t> flush_ssd_;
std::vector<uint64_t> flush_rocksdb_;
std::vector<uint64_t> flush_update_;
...
int GetTotalFileSize() {
    int a =  system("du -hs /home2/du/work/KV_NVMe/mnt > /home2/du/work/KV_NVMe/1.txt");
    // read File
    int dir_size = 0;
...
Status FlushJob::WriteLevel0Table() {
  ...
  volatile int * sul_status = nullptr;
  int sul_count = 0;
  int du_last_seq = 0;
...

  {
    ...
    for (MemTable* m : mems_) {
      ...
      int memt_seq_start = m->GetEarliestSequenceNumber();
      if (seq_smallest == 0 || memt_seq_start < seq_smallest) {
        seq_smallest = memt_seq_start;
      }
      int local_seq = (int) m->GetLastSequenceNumber();
      printf("\t\t ** My sequence number of Memtable %d %d \n", local_seq, kv_worker_->getMaxSeq());
      if (du_last_seq < local_seq) {
        du_last_seq = local_seq;
      }
    }
    /*
    for (MemTable* m : mems_) {
        printf("flush Job Memtable..St DEBUGGING PARTTTTTTTTTT\n");
        InternalIterator* du_iter;
        du_iter = m->NewIterator(ro, &arena);
        for (du_iter->SeekToFirst(); du_iter->Valid(); du_iter->Next()) {
            if (du_iter->value().size() >= 4096) {
                std::cout << "my key " << du_iter->key().data() << std::endl;
            }
            //const Slice& key = du_iter->key();
            //const Slice& value = du_iter->value();
        }
     }
     */
    ...
    if (kv_ssd_ != nullptr && (kv_ssd_enable_ == 3 || kv_ssd_enable_ == 4)) {
        const uint64_t du_micros = clock_->NowMicros();
        kv_index_->sulCreate(seq_smallest);
        kv_index_->sulFlush(seq_smallest, seq_smallest, my_buffer, my_key); // seq start - seq end
        const uint64_t du_micros1 = clock_->NowMicros();
        sul_count = (int ) my_buffer.size();
        sul_status = (int *)malloc(sizeof(int) * sul_count);
        memset((void *) sul_status, 2, sul_count); 
        kv_ssd_->sulFlush(my_buffer, my_key, (int *) sul_status, 0);
        const uint64_t du_micros2 = clock_->NowMicros();
       
        uint64_t rocksdb_size = 0;
        uint64_t last_level_size = 0;
        auto vstorage = cfd_->current()->storage_info();
        int last_level_number = -1;
        int zero_bytes_count = 0;
        for (int level = 0; level < vstorage->num_levels(); ++level) {
            uint64_t get_bytes = (uint64_t) vstorage->NumLevelBytes(level);
            rocksdb_size += get_bytes;
            if (get_bytes == 0) {
                zero_bytes_count ++;
            } else {
                last_level_size = get_bytes;
                last_level_number = last_level_number + 1 + zero_bytes_count;
                zero_bytes_count = 0;
            }
            std::cout << "level " << level << " files " << vstorage->NumLevelFiles(level) << " Bytes " << vstorage->NumLevelBytes(level) << " get_bytes " << get_bytes << " last level num " << last_level_number << std::endl;
        }
       
        uint64_t kv_size = total_rocksdb_size_ - rocksdb_size;
        kv_ssd_->kvUpdateUtilization(rocksdb_size, kv_size, last_level_size, last_level_number); 
        kv_ssd_->kvstrUpdate();
        
        const uint64_t du_micros3 = clock_->NowMicros();
        
        uint64_t index_lat = du_micros1 - du_micros;
        uint64_t write_lat = du_micros2 - du_micros1;
        uint64_t info_lat = du_micros3 - du_micros2;
        
        RecordTimeToHistogram(stats_, KV_FLUSH_INDEX, index_lat);
        RecordTimeToHistogram(stats_, KV_FLUSH_WRITE, write_lat);
        RecordTimeToHistogram(stats_, KV_FLUSH_INFO, info_lat);
  ...
  if (kv_ssd_ != nullptr && (kv_ssd_enable_ == 3 || kv_ssd_enable_ == 4)) { 
      uint64_t du_micros = clock_->NowMicros(); 
      if (sul_status != nullptr) {
        kv_ssd_->sulFlushCheck(sul_status, sul_count);
        free((void *) sul_status);
      }
      kv_index_->sulDelete(0, 1);
      uint64_t du_end_micros = clock_->NowMicros(); 
      printf("After sulFlush Case Wait Time %lld RocksDB Total Flush Time %lld \n", (long long int) (du_end_micros - du_micros), (long long int)(du_end_micros-start_micros));
      //kv_index_->sulPrint();
  
      // Wait for .. Sequence Wait!!!!!!!!!!!!!!!!
      // only for enable_ = 4
      if (kv_ssd_enable_ == 4) {
          // MemTableSeq vs KV Write Seq 
          while (du_last_seq > (100 + kv_worker_->getMaxSeq())) {
              //if (rand() % 100000) {
              //    printf("Hang.. %d %d \n", du_last_seq, kv_worker_->getMaxSeq());
              //}
          }
      }
  }
