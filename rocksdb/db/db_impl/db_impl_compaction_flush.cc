  ...
  *made_progress = false;
  ...
  if (!c) {
    ...
    *made_progress = true;
    ...
    std::cerr << "Level " << start_level << "\t" << start_micros << std::endl;  
    compaction_job.Prepare();
    ...
    *made_progress = true;
    TEST_SYNC_POINT_CALLBACK("DBImpl::BackgroundCompaction:AfterCompaction",
                             c->column_family_data());
    const uint64_t end_micros = immutable_db_options_.clock->NowMicros();
    std::cerr << "Level " << start_level << "\t" << start_micros << "\t" << end_micros - start_micros << std::endl;  
  }
