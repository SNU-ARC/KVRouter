...
// Current Do not wait Finish SSD Write 
bool DBImpl::CheckBatchSizeForKV(WriteThread::Writer * w) { //, int * status) {
    if (kv_ssd_enable_ != 3) return false;
    ...
    char * buffer = (char*)WriteBatchInternal::Contents(w->batch).data();
    int key_size = 16;
    ...
    if (batch_size >= 4096) {
        ...
        /*
        if (status == nullptr) {
            printf("may be error status is nullptr\n");
        }
        *status = 2;
        */
        kv_ssd_->kv_put_buffer_async(du_key.data(), du_key.size(), du_value.data(), du_value.size(), (int *) nullptr); 
        kv_index_->keyInsert((char *) du_key.data(), 16, 444, du_value.size());  
    } else {
...
Status DBImpl::PipelinedWriteImpl(const WriteOptions& write_options,
  ...
  if (w.state == WriteThread::STATE_GROUP_LEADER) {
    ...
    last_batch_group_size_ =
        write_thread_.EnterAsBatchGroupLeader(&w, &wal_write_group, du_status);
    ...
    //int kk = 0 ; 
    //size_t du_sequence = current_sequence;
    if (w.status.ok()) {
      // TODO: this use of operator bool on `tracer_` can avoid unnecessary lock
      // grabs but does not seem thread-safe.
      if (tracer_) {
      ...
      //if (kk > 1 || kk == 0) printf("kk value is %d  \n", kk);
      write_thread_.UpdateLastSequence(current_sequence + total_count - 1);
      //printf("prev Seq %ld Next sequence %ld total %ld\n", du_sequence, next_sequence, total_count); 
    }
...
#if (KV_MIGRATION == 1)
      kv_worker_->kvEnqueue((char *) (buffer + 14), 16, w.sequence, (char *) (buffer+32), value_size, write_options.direction);
#else
      kv_worker_->kvEnqueue((char *) (buffer + 14), 16, w.sequence, (char *) (buffer+32), value_size);
#endif
  }
  ...
  /*
  auto du_start = immutable_db_options_.clock->NowMicros();
  for (int i = 0 ; i < 64 ;) {
      if (du_status[i] == 2) {
        auto du_mid = immutable_db_options_.clock->NowMicros();
        auto du_gap = du_mid - du_start;
        if (du_gap > 1024*1024) {
            printf("%s Wait Time is too long... %lld may be seconds \n", __func__, (long long int)du_gap);
            for (int q = 0 ; q < 64 ; q ++) {
                printf("%d\t", du_status[q]);
            }
            printf("\n");
            du_start = immutable_db_options_.clock->NowMicros();
        }
        continue;
      }
      i++;
  ...
  */
  //printf("Wait Time...is %lld\n", (long long int) ( immutable_db_options_.clock->NowMicros() - du_start));
...
Status DBImpl::PreprocessWrite(const WriteOptions& write_options,
  ...
  InstrumentedMutexLock l(&log_write_mutex_);
  // fill : kvcomp_enable = false         other true
  // warmup : phase_ = 1 or 10            other 0
  bool disable_kv_sync = false;
  if (kvssd_compaction_enable_ == false || warmup_phase_ != 0) {
      disable_kv_sync = true;
      if (rand() % 30000 == 0) {
          printf("disable kv sync %d %d %d \n", (int)disable_kv_sync, (int) kvssd_compaction_enable_, (int) warmup_phase_);
      }
  }

  if (status.ok() && log_context->need_log_sync && (disable_kv_sync == false)) {
...
Status DBImpl::SwitchMemtable(ColumnFamilyData* cfd, WriteContext* context) {
  ...
  // if (kv_ssd_enable_ == 4) DU 
  //printf("\t\t DU Set Last Sequence %ld\n", versions_->LastSequence());
  cfd->mem()->SetLastSequenceNumber(versions_->LastSequence());
  printf("\t\t Setup First Sequence Number !! %ld \n", versions_->LastSequence());
  //memtable_info.last_seqno = versions_->LastSequence();
...
Status DB::Put(const WriteOptions& opt, ColumnFamilyHandle* column_family,
  ...
  //if (value.size() >= 4096) 
  //    std::cout << "Insert key " << key.ToString(false) << std::endl;
  if (!s.ok()) {
    return s;
  }
  // DU  
  Status qq = Write(opt, &batch);
  //if (value.size() >= 4096) 
  //std::cout << "Done" << key.ToString(false) << std::endl;
  return qq;
