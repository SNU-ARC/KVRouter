...
//#include "db/kvssd/kv_index.h"
//#include "utilities/kvssd/kv_ssd.h"

namespace ROCKSDB_NAMESPACE {
class KVIndex;
class KV_SSD; 

bool du_enable_ = false;
bool kvssd_compaction_enable_ = false;
bool kvssd_algo_value_ = 1;

int kvssd_count_ = 1;
int kv_ssd_enable_ = 0;
int kv_comp_opt_ = 0; 
int kv_seq_opt_ = 0; 
uint64_t du_kv_range_ = 0;
bool du_trivial_disable_ = false;
KVIndex * kv_index_;

int kv_ssd_threshold_ratio_ = 2;

WorkerQueue * kv_worker_;
KV_SSD * kv_ssd_;
KV_SSD * kv_ssd_new_;
KVComp * kv_comp_;

long long int total_rocksdb_size_ = 0; // MB

bool read_index_[64] = {false};
...
//////////////////////////
// Statistics
typedef struct lat_info {
    long long int lat_;
    int level_;
}lat_info;
std::vector<lat_info> rocks_read_lat_[16];
std::vector<long long int> kv_read_lat_[16];

uint64_t kv_anal_time_prev_[16] = {0};
uint64_t rocks_anal_time_prev_[16] = {0};

bool new_cmp (const lat_info & a, const lat_info & b) {
    if (a.lat_ < b.lat_) return true;
    else return false;
}
/*
std::vector<long long int> rocks_read_lat_[16];
std::vector<long long int> kv_read_lat_[16];

long long int rocks_read_lat_[16][512];
long long int kv_read_lat_[16][512];
long long int rocks_read_cnt_[16];
long long int kv_read_cnt_[16];
*/

long long int read_qps_count_[16] = {0}; 
long long int kv_qps_count_[16] = {0}; 
long long int rocks_qps_count_[16] = {0}; 
...
DBImpl::DBImpl(const DBOptions& options, const std::string& dbname,
  ...
  printf("Create kv_ssd series ! %d\n", kvssd_count_);
  kv_ssd_ = new KV_SSD();
  if (kvssd_count_ == 2) {
    kv_ssd_new_ = new KV_SSD(12);
  } else {
    kv_ssd_new_ = nullptr;
  }
  kv_index_ = new KVIndex();
  kv_index_->allocateIndex(true, 0, 0, 0);
  kv_comp_ = new KVComp(kv_ssd_);
  kv_comp_->allocateIndex(true, 0, 0, 0);

  kv_worker_ = new WorkerQueue();
  kv_worker_->set_ssd(kv_ssd_);
  kv_worker_->set_index(kv_index_);
...
void DBImpl::DUStat() {
    printf("Report  here * ===================\n");
    printf("Start KV SSD Report \n");
    kv_ssd_->strPrint();
    long long kv_count = 0; 
    long long kv_capa = 0; 
    kv_index_->getCapacity(&kv_count,  &kv_capa);
    long long kvcomp_count = 0; 
    long long kvcomp_capa = 0; 
    KVIndex * comp_index = kv_comp_->getKVIndex();
    comp_index->getCapacity(&kvcomp_count,  &kvcomp_capa);
    printf("kv count & capa %lld %lld comp %lld %lld\n", kv_count, kv_capa, kvcomp_count, kvcomp_capa); 
}
...
Status DBImpl::GetImpl(const ReadOptions& read_options, const Slice& key,
...
#if (KV_MIGRATION == 1)
  int thread_id = read_options.kv_ssd_id; 
  long long int lat_anal_start = (long long int) immutable_db_options_.clock->NowMicros();
  int hit_level = 0;
  if ((rocks_qps_count_[thread_id] + kv_qps_count_[thread_id]) > 10000) {
      printf("rocks_qps_count %lld kv_qps_count %lld\n", rocks_qps_count_[thread_id], kv_qps_count_[thread_id]);
      rocks_qps_count_[thread_id] = 0;
      kv_qps_count_[thread_id] = 0;
  }
#endif
  if (!done) {
      // KV SSD Check 
      if (kv_ssd_ != nullptr && kv_index_ != nullptr && (kv_ssd_enable_ == 3 || kv_ssd_enable_ == 4)) {
          uint64_t index_start = immutable_db_options_.clock->NowMicros();
          bool find_sul = kv_index_->keyFind((char *) key.data(), key.size(), kv_seq_opt_, read_options.kv_ssd_id);
          uint64_t index_end = immutable_db_options_.clock->NowMicros();
          RecordTimeToHistogram(stats_, KV_L0_INDEX, index_end - index_start);
          if (find_sul) {
              uint64_t read_start = immutable_db_options_.clock->NowMicros();
              char * value_p = (char*) malloc(16384);
              size_t size = 0;
              if (get_impl_options.get_value) {
                  ...
                  if (kv_seq_opt_ == 1) {
                      if (rand() % 1000 == 0) 
                          printf("L0 Hit here \n");
                      if (read_options.l0_hit == nullptr) {
                          printf("L0 is null WTF? !!!!!!!!!!!!! TODOOOOOOOOOOOOOOOO\n");
                      } else {
                          *(read_options.l0_hit) = true;
                      }
#if (KV_PREFETCH == 1)
                      prefetch_hit = kv_index_->checkPrefetch(read_options.kv_ssd_id, key.data(), value_p);
#endif
                  }

                  if (prefetch_hit != true) {
                      uint64_t rrrr_start = immutable_db_options_.clock->NowMicros();
                      size = kv_ssd_->kv_get_buffer_sync(key.data(), key.size(), value_p, 16384, -1);
                      uint64_t rrrr_end = immutable_db_options_.clock->NowMicros();
                      RecordTimeToHistogram(stats_, KV_L0_SSD, rrrr_end - rrrr_start);
                      //////////////////////////////////////////
                      // Only for Statistics 
#if (KV_MIGRATION == 1) 
                      kv_qps_count_[thread_id] ++; 
                      long long int lat_anal_end = (long long int) immutable_db_options_.clock->NowMicros();
                      
                      if ((lat_anal_end - kv_anal_time_prev_[thread_id]) > 20*1024*1024) { // Every 20 seconds 
                          kv_read_lat_[thread_id].push_back(lat_anal_end - lat_anal_start);
                          if (kv_read_lat_[thread_id].size() >= 1000) {
                              std::sort(kv_read_lat_[thread_id].begin(), kv_read_lat_[thread_id].end());
                              std::vector<long long int>::iterator it = kv_read_lat_[thread_id].begin();
                              long long int sum_of_value = 0;
                              int index = 0;
                              for ( ; it < kv_read_lat_[thread_id].end() ; it ++) {
                                  sum_of_value += *it;
                                  if (index > 800) {
                                      kv_max_[thread_id] = *it;
                                  }
                                  index ++;
                              }
                              kv_avg_[thread_id] = (float) sum_of_value / 1000; 
                              printf("KV Average %f Max %f Rocks Average %f max %f \n", kv_avg_[thread_id], kv_max_[thread_id], rocks_avg_[thread_id], rocks_max_[thread_id]);
                              kv_read_lat_[thread_id].clear();


                              kv_anal_time_prev_[thread_id] = lat_anal_end;
                          }
                      }
#if (KV_MIGRATION_RR == 1)
                      // for random read
                      /*
                      if (rocks_avg_[thread_id] > kv_avg_[thread_id]) { // KV Fast
                          if (rocks_qps_count_[thread_id] > kv_qps_count_[thread_id]) { // Rocks QPS High
                              if (kv_worker_->GetMigrationStop() == true) {
                                  if (read_options.migration == nullptr) {
                                      printf("Migration fail ! why nullptr\n");
                                  } else {
                                      // *(read_options.migration) = true;
                                      *(read_options.migration) = 2;
                                      if (rand() % 1000 == 0) { 
                                          printf("Migrate to RocksDB for random read rocks qps %lld kv qps %lld rocks avg %f kv avg %f\n",
                                                  rocks_qps_count_[thread_id], kv_qps_count_[thread_id], rocks_avg_[thread_id], kv_avg_[thread_id]);
                                      }
                                  }
                              }
                      ...
                      */ 
                      // for sequenial read 
                      if (rocks_max_[thread_id] < kv_avg_[thread_id]) { // KV is rrrrrrrrealllllly Fast
                          if (rocks_qps_count_[thread_id] > kv_qps_count_[thread_id]) { // Rocks QPS High
                              float lat_gap = lat_anal_end - lat_anal_start;
                              if (lat_gap > kv_max_[thread_id]) {
                                  if (read_options.migration == nullptr) {
                                      printf("Migration fail ! why nullptr\n");
                                  } else {
                                      // *(read_options.migration) = true;
                                      *(read_options.migration) = 2;
                                      if (rand() % 1000 == 0) { 
                                          printf("Migrate to RocksDB for random read rocks qps %lld kv qps %lld rocks avg %f kv avg %f rocks max %f kv max %f \n",
                                                  rocks_qps_count_[thread_id], kv_qps_count_[thread_id], rocks_avg_[thread_id], kv_avg_[thread_id], rocks_max_[thread_id], kv_max_[thread_id]);
                                      }
                                  }
                              }
                  ...
                  printf("May be error Case %s\n", __func__);
              }
              uint64_t read_end = immutable_db_options_.clock->NowMicros();
              skip_memtable = true;
              done = true;

              RecordTick(stats_, KVSSD_HIT);
              RecordTick(stats_, KV_L0_HIT);
              RecordTimeToHistogram(stats_, KV_L0, read_end - read_start);
  ...
  if (!done) {
    ...
    char du_key[17];
    memset(du_key, 0, 17);
    memcpy(du_key, key.data(), 16);
    status = 0;
    kv_ssd_->kv_get_buffer_async(key.data(), 17, value_p, 16384, nullptr);
    */
    //kv_ssd_->kv_get_buffer_async(key.data(), 17, value_p, 16384, &status);
    PERF_TIMER_GUARD(get_from_output_files_time);
#if (KV_MIGRATION == 1) 
    hit_level = sv->current->Get(
        ...
        get_impl_options.get_value, read_options.kv_ssd_id, read_options.last_hit);
#else
    sv->current->Get(
        read_options, lkey, get_impl_options.value, get_impl_options.columns,
        timestamp, &s, &merge_context, &max_covering_tombstone_seq,
  ...
  //free(value_p); 
  // TODO
  // It is only foudn in KV SSD 
  if (kv_ssd_->isCompactionEnable() && s == Status::NotFound() && (kv_ssd_enable_ == 3 || kv_ssd_enable_ == 4)) {
      ReturnAndCleanupSuperVersion(cfd, sv);
      RecordTick(stats_, KV_LAST_HIT);
      return Status::OK();
  }

#if (KV_MIGRATION == 1) 
  long long int lat_anal_end = (long long int) immutable_db_options_.clock->NowMicros();
  long long int anal_latency = lat_anal_end - lat_anal_start;

  rocks_qps_count_[thread_id] ++; 
  if ((float) anal_latency > rocks_max_[thread_id]) {
      
      if (rocks_qps_count_[thread_id] > kv_qps_count_[thread_id]) {
          // Rocks Max < KV Average == > It is sequential read case
          if (rocks_max_[thread_id] > kv_avg_[thread_id]) { // Try to except sequential case 
              if (read_options.migration == nullptr) {
                  printf("Migration fail ! why nullptr\n");
              } else {
                  *(read_options.migration) = 1;
                  // *(read_options.migration) = true;
              }
              if (rand() % 1000 == 0) 
                  printf("Succ latency %lld or %f max %f\n", anal_latency, (float)anal_latency, rocks_max_[thread_id]);
          }
      }
      ...
      */
      ...
      //if (rand() % 1000 == 0) 
      //    printf("Fail latency %lld or %f max %f\n", anal_latency, (float)anal_latency, rocks_max_[thread_id]);
  }

  if ((lat_anal_end - rocks_anal_time_prev_[thread_id]) > 20*1024*1024) { // Every 20 seconds 
      lat_info new_info;
      new_info.lat_ = anal_latency;
      new_info.level_ = hit_level;
      rocks_read_lat_[thread_id].push_back(new_info);
      //rocks_read_lat_[thread_id].push_back(lat_anal_end - lat_anal_start);
      if (rocks_read_lat_[thread_id].size() >= 1000) {
          std::sort(rocks_read_lat_[thread_id].begin(), rocks_read_lat_[thread_id].end(), new_cmp);
          //std::sort(rocks_read_lat_[thread_id].begin(), rocks_read_lat_[thread_id].end());
          std::vector<lat_info>::iterator it = rocks_read_lat_[thread_id].begin();
          //std::vector<long long int>::iterator it = rocks_read_lat_[thread_id].begin();
          long long int sum_of_value = 0;
          int index = 0;
          for ( ; it < rocks_read_lat_[thread_id].end() ; it ++) {
              sum_of_value += it->lat_;
              if (index == 800) {
              //if (index == 800) {
                  rocks_max_[thread_id] = it->lat_;
              }
              index ++;
          }
          rocks_avg_[thread_id] = (float) sum_of_value / 1000;
          printf("Rocks Average %f Max %f KV Avg %f Max %f \n", rocks_avg_[thread_id], rocks_max_[thread_id], kv_avg_[thread_id], kv_max_[thread_id]);
          rocks_read_lat_[thread_id].clear();

          rocks_anal_time_prev_[thread_id] = lat_anal_end;
      }
  }
  ///////////
  if (hit_level == 0) {
    RecordTick(stats_, HIT_L0);
  } else if (hit_level == 1) {
    RecordTick(stats_, HIT_L1);
  } else if (hit_level == 2) {
    RecordTick(stats_, HIT_L2);
  } else if (hit_level == 3) {
    RecordTick(stats_, HIT_L3);
  } else if (hit_level == 4) {
    RecordTick(stats_, HIT_L4);
  } else if (hit_level == 5) {
    RecordTick(stats_, HIT_L5);
  }
