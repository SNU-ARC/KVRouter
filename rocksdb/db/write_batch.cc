...
struct BatchContentClassifier : public WriteBatch::Handler {
  ...
  Status PutCF(uint32_t, const Slice&, const Slice&, uint8_t direction=0) override {
    if (direction) {}
...
Status WriteBatch::Iterate(Handler* handler, uint8_t direction) const {
...
Status WriteBatchInternal::Iterate(const WriteBatch* wb,
  ...
  //std::cout << "Iterate " << WriteBatchInternal::Contents(wb).ToString(false) << std::endl;
...
#if (KV_MIGRATION == 1)
        s = handler->PutCF(column_family, key, value, direction);
#else
...
class MemTableInserter : public WriteBatch::Handler {
 ...
 public:
  ...
  Status PutCFImpl(uint32_t column_family_id, const Slice& key,
                   ...
                   const ProtectionInfoKVOS64* kv_prot_info, uint8_t direction = 0) {
  ...
  Status PutCF(uint32_t column_family_id, const Slice& key,
               const Slice& value, uint8_t direction=0) override {
    if (direction) {}
...
#if (KV_MIGRATION == 1)
      ret_status = PutCFImpl(column_family_id, key, value, kTypeValue,
                             nullptr /* kv_prot_info */, direction);
#else
      ret_status = PutCFImpl(column_family_id, key, value, kTypeValue,
                             nullptr /* kv_prot_info */);
#endif
...
Status WriteBatchInternal::InsertInto(
    ...
    bool concurrent_memtable_writes, bool seq_per_batch, bool batch_per_txn, uint8_t direction) {
...
Status WriteBatchInternal::InsertInto(
    ...
    bool batch_per_txn, bool hint_per_batch, uint8_t direction) {
    if (direction) {}
#ifdef NDEBUG
  (void)batch_cnt;
#endif
...
Status WriteBatchInternal::InsertInto(
    ...
    bool* has_valid_writes, bool seq_per_batch, bool batch_per_txn, uint8_t direction) {
    if (direction) {}
...
class ProtectionInfoUpdater : public WriteBatch::Handler {
 public:
  ...
  Status PutCF(uint32_t cf, const Slice& key, const Slice& val, uint8_t direction=0) override {
      if (direction) {}
