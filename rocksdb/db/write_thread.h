...
class WriteThread {
 public:
  ...
  size_t EnterAsBatchGroupLeader(Writer* leader, WriteGroup* write_group, volatile int * status = nullptr);
  void CheckKVSSD(char * buffer, int key_size);
  bool WriteToKVSSD(char * buffer, int key_size, int value_size, int sequence, int writer_id, volatile int * status);
  uint64_t DUMicro();
