...
Status MemTable::Add(SequenceNumber s, ValueType type,
                     ...
                     MemTablePostProcessInfo* post_process_info, void** hint, uint8_t direction) {
  ...
  const uint32_t encoded_len = VarintLength(internal_key_size) +
                               ...
                               val_size + moptions_.protection_bytes_per_key + sizeof(uint8_t);
#else 
  const uint32_t encoded_len = VarintLength(internal_key_size) +
                               internal_key_size + VarintLength(val_size) +
  ...
  memcpy(p, &direction, 1);
  assert((unsigned)(p  - buf + moptions_.protection_bytes_per_key + 1) ==
