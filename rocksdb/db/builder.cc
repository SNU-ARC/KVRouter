...
Status BuildTable(
  ...
  if (iter->Valid() || !range_del_agg->IsEmpty()) {
    ...
    for (; c_iter.Valid(); c_iter.Next()) {
      ...
      //if (blob_creation_reason == BlobFileCreationReason::kFlush && (c_iter.value().size() >= 4096) && kv_ssd_enable_ == 4) {
      if (blob_creation_reason == BlobFileCreationReason::kFlush && (c_iter.value().size() >= (size_t) kv_worker_->getDumpSize()) && kv_ssd_enable_ == 4) {
          //printf("Hello...I am here ! %s size %ld \n", __func__, c_iter.value().size());
      } else {
#if (KV_MIGRATION == 1)
          if (blob_creation_reason == BlobFileCreationReason::kFlush && kv_ssd_enable_ == 4) {
              size_t value_size = value.size();
              uint8_t direction = 0;
              memcpy((void *) &direction, value.data() + value_size, sizeof(uint8_t));
              if (direction == 1) {
                  continue; // This value is stored in KVSSD
              } else if (direction == 2) {
                  // Nothing
              } else if (direction == 0) {
                  // Nothing
              } else {
                  printf("Error Case %s Why Direction is ??? %d\n", __func__, (int)direction);
              }
          }
#endif
