...
class FilePicker {
 public:
  FilePicker(const Slice& user_key, const Slice& ikey,
    ...
    exist_in_kvssd_ = false;
    need_check_kvssd_ = false;
  ...
  void SetKVExist() { exist_in_kvssd_ = true; }
  bool GetKVCheck() { return need_check_kvssd_; }
  int GetCurrentLevel() const { return curr_level_; }

  long long int prefetch_miss_count = 0;
  long long int prefetch_hit_count = 0;
  FdWithKeyRange* GetNextFile(int thread_id = 0, bool * last_hit=nullptr) {
    while (!search_ended_) {  // Loops over different levels.
        if (exist_in_kvssd_) {
            // Level Check First
            if (curr_level_ >= (unsigned int) kv_ssd_->getLastLevel()) {
#if (0)
                int target_value = kv_ssd_->getScore(); 
                int du_key_size = 17;
                char compare_key[17];
                memcpy(compare_key, user_key_.data(), 12);
                compare_key[12] = '\0';
                int compare_value = 0;
                sscanf(compare_key, "%d", &compare_value); 

                if (compare_value < target_value) {
                    need_check_kvssd_ = true;
                    if (true) {
                        char * value_p = (char*) malloc(16384);
                        memset(compare_key, 0, du_key_size);
                        memcpy(compare_key, user_key_.data(), 16);
                        kv_ssd_->kv_get_buffer_sync(compare_key, du_key_size, value_p, 16384, 1234);
                        free(value_p);
                        //std::cout << "Take key from KV " << compare_key << "\t" << compare_value << std::endl;
                    }
                    return nullptr;
                }
#else
                if (thread_id && last_hit) {}
                // Seq Opt 1: Prefetch specific number of reqeusts 
                bool find_sul = kv_comp_->keyFind((char *) user_key_.data(), 16, kv_seq_opt_, thread_id);
                if (find_sul) {
                    need_check_kvssd_ = true;
                    bool prefetch_hit = false;
                    //PreE * element = nullptr;
                    char * value_p = (char*) malloc(16384);
                    if (kv_seq_opt_ == 1) {
                        if (rand() % 1000 == 0) 
                            printf("L4 Hit ! Here \n");
                        if (last_hit == nullptr) {
                            printf("L4 is null WTF? !!!!!!!!!!!!! TODOOOOOOOOOOOOOOOO\n");
                        } else {
                            *(last_hit) = true;
                        }
                        prefetch_hit = kv_comp_->checkPrefetch(thread_id, user_key_.data(), value_p);
                    }
                    ...
                    if (prefetch_hit != true) {
                        ...
                        memcpy(compare_key, user_key_.data(), 16);
                        kv_ssd_->kv_get_buffer_sync(compare_key, 17, value_p, 16384, 1234);
                    } 
                    free(value_p);
                    ...
                    if (kv_seq_opt_ == 1) {
                        if (prefetch_hit) {
                            prefetch_hit_count ++;
                            if (prefetch_hit_count % 1000 == 0) {
                                printf("PREFETCH HIT in LAST Level %lld Miss %lld \n", prefetch_hit_count, prefetch_miss_count);
                            }
                        } else {
                            prefetch_miss_count ++;
                        }
 ...
 private:
  ...
  bool exist_in_kvssd_;
  bool need_check_kvssd_;
...
int Version::Get(const ReadOptions& read_options, const LookupKey& k,
//void Version::Get(const ReadOptions& read_options, const LookupKey& k,
                  ...
                  bool* is_blob, bool do_merge, int thread_id, bool * last_hit) {
  ...
  // DU Table Reader
  if ((kv_ssd_enable_ == 4 || kv_ssd_enable_ == 3) && kv_ssd_->isCompactionEnable()) {
      fp.SetKVExist();
  }
  ...
  while (f != nullptr) {
    ...
    *status = table_cache_->Get(
    ...
    f = fp.GetNextFile(thread_id, last_hit);
    
    // TODO DU Check
    if (f == nullptr) {
        if (kv_ssd_->isCompactionEnable() && (kv_ssd_enable_ == 3 || kv_ssd_enable_ == 4) && fp.GetKVCheck()) {
            //std::cout << " I am Setup this Key from KV" << std::endl;
            *status = Status::NotFound();
            return fp.GetHitFileLevel();
        }
    }
  }
  if (db_statistics_ != nullptr) {
...
void SortFileByOverlappingRatio(
    ...
    std::vector<Fsize>* temp, int max_level) {
  ...
  int change_number = 0;
  int index_number = 0;
  bool du_compaction_enable  = false;
  //bool range_set = false;
  bool last_level_set = false;
  char kv_first[17];
  char kv_last[17];
  if (kv_ssd_ != nullptr && (kv_ssd_enable_ == 3 || kv_ssd_enable_ == 4)) {
    if (kv_ssd_->isCompactionEnable() && kvssd_compaction_enable_) {
        du_compaction_enable = true;
    }
  }

  bool try_to_warm = false;
  int warmup_count = 0;
  ...
  for (auto& file : files) {
    ...
    if (du_compaction_enable) {
        int gap_with_last_level = (level == 0) ? 100 : kv_ssd_->getLastLevel() - level;
        /*
        bool last_level_check = false;
        if ((level != 0) && (level == kv_ssd_->getLastLevel())) {
            last_level_check = true; 
        }
        */
    
        if (gap_with_last_level <= 1 && print_or_not && index_number < 200) {
            std::cout << __func__ << "\t" << 
                level << "\t" <<
                max_level << "\t" <<
                gap_with_last_level << "\t" <<
                kv_ssd_->getLastLevel() << "\t" <<
                warmup_phase_ << "\t" <<
                //last_level_check << "\t" <<
                index_number << "\t" << 
                file->smallest.user_key().ToString(false) << "\t" << 
                file->largest.user_key().ToString(false) << "\t" <<
                file_to_order[file->fd.GetNumber()] << std::endl;
        }
#if (1)
        if (gap_with_last_level == 1 && warmup_phase_ != 0) {
            if (change_number < 10 && file_to_order[file->fd.GetNumber()] != 0) {
                if (print_or_not) 
                    std::cout << __func__ << "\tKey " <<  file->smallest.user_key().ToString(false) << "\tchange number\t" << change_number << "\t" <<file_to_order[file->fd.GetNumber()] << "\t";
                file_to_order[file->fd.GetNumber()] = change_number++;
            } else {
                if (print_or_not) 
                    std::cout << __func__ << "\tKey " <<  file->smallest.user_key().ToString(false) << "\tchange number\t" << change_number << "\t" <<file_to_order[file->fd.GetNumber()] << "\t";
                file_to_order[file->fd.GetNumber()] = 10000;
            }
            if (print_or_not) 
                std::cout << "Changed to " << file_to_order[file->fd.GetNumber()] << std::endl;
        }
        if (gap_with_last_level == 0 && warmup_phase_ != 0) {
            try_to_warm = true;
            int target_value = kv_ssd_->getScore();
            int small_number = 0;
            char small_char[16];
            memcpy(small_char, (void *) file->smallest.user_key().data(), 12);
            small_char[12] = '\0';
            sscanf(small_char, "%d", &small_number); 
            //printf("gap_with last level %d small number %d target_value %d\n", gap_with_last_level, small_number, target_value);
            if (small_number < target_value) {
                warmup_count ++;
            }
        }
#endif
  ...
  if (kv_ssd_enable_ == 3 || kv_ssd_enable_ == 4) {
      if (try_to_warm || (kv_ssd_->getScore() == 0)) {
          if (warmup_count <= 10) {
              warmup_phase_ = 2;
              printf("Now Successfully warmup done! warmup count %d score %d \n", warmup_count, kv_ssd_->getScore());
          } else {
              printf("Warmup Phase is still 1 !? count %d score %d \n", warmup_count, kv_ssd_->getScore());
          }
      }
  }

  if (du_compaction_enable && last_level_set) {
    std::cout << " Expand Range !! so this is First Case ! DU Check" << std::endl;
    std::cout << __func__ << "\t ** " << kv_first << " \t ** " << kv_last << std::endl;
    // TODO : only expand Range
    // kv_comp_->expandRange(kv_first, kv_last);
  }
  ...
  // temp : Sorted by file size (Large -> -> -> small)
  // file_to_order : priority by score ! 
  ...
  std::vector<Fsize>::iterator it; 
  std::cout << " ******* After Sort File Order ******** " << std::endl;
  for (it = temp->begin() ; it != temp->end() ; it ++) { 
      std::cout << __func__ << "\t" << 
          level << "\t" <<
          index_number << "\t" << 
          it->file->smallest.user_key().ToString(false) << "\t" << 
          it->file->largest.user_key().ToString(false) << std::endl;
      index_number ++ ; 
  }
  */
}
    ...
    switch (ioptions.compaction_pri) {
      ...
      case kMinOverlappingRatio:
        SortFileByOverlappingRatio(*internal_comparator_, files_[level],
                                   ...
                                   num_non_empty_levels_, options.ttl, &temp, last_level);
        break;
      case kRoundRobin:
...
void VersionStorageInfo::GetOverlappingInputsRangeBinarySearch(
  ...
  if (kv_comp_ != nullptr) {
      /*
      auto du_smallest = files_[level][start_index]->smallest.user_key().data();
      auto du_largest = files_[level][end_index-1]->largest.user_key().data();
      // smallest and largest check here! 
      if (kv_comp_->checkRange((char *) du_smallest, (char *) du_largest, 16)) {
          printf("%s Now I try to setRange KV Compaction !! \n", __func__);
          kv_comp_->setRange((char *) du_smallest,  (char *) du_largest, 16);
      } else { // out of range do not care this point
      }
      */
  }
...
bool VersionStorageInfo::RangeMightExistAfterSortedRun(
  ...
  for (int level = last_level + 1; level < num_levels(); level++) {
    ...
    if (files_[level].size() > 0 &&
        ...
        /*
        std::cout << "\t\t\t ** It is reason of false bottom \t"  << __func__ <<
            last_level << "\t" <<
            level << "\t" <<
            num_levels() << "\t" << std::endl;
        */
