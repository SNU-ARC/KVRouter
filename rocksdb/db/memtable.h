...
class MemTable {
 public:
  ...
  Status Add(SequenceNumber seq, ValueType type, const Slice& key,
             ...
             void** hint = nullptr, uint8_t direction = 0);
  ...
  void SetLastSequenceNumber(SequenceNumber last_seqno) {
    return last_seqno_.store(last_seqno, std::memory_order_relaxed);
  }
 ...
 private:
  ...
  std::atomic<SequenceNumber> last_seqno_;
