...
int write_to_kv_cnt_;
std::vector<uint64_t> write_to_kv_index_;
std::vector<uint64_t> write_to_kv_ssd_;
...
//std::vector<uint64_t> write_to_kv_index_;
//std::vector<uint64_t> write_to_kv_ssd_;
uint64_t WriteThread::DUMicro() {
    uint64_t us = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count();
    return us;
}

void WriteThread::CheckKVSSD(char * buffer, int key_size) {
    Slice du_key(buffer + 14, key_size);
        
    if (kv_index_ != nullptr && kv_ssd_ != nullptr) {
        bool find = kv_index_->keyDelete((char *) du_key.data(), 16);  
        if (find) {
            printf("Delete Key is success may be value size is changed.....?\n");
        }
    }
}


bool WriteThread::WriteToKVSSD(char * buffer, int key_size, int value_size, int sequence, int writer_id, volatile int * status) {
    //bool dbg = true;
    //char * qwer = (char *)WriteBatchInternal::Contents(w->batch).data(); 
    //int key_size = 16;
    //int value_size = batch_size - 32;
    Slice du_key(buffer + 14, key_size);
    Slice du_value(buffer + 32, value_size);

    if (writer_id && sequence) {}

    //std::cout << du_key.ToString(false) << std::endl;
    if (kv_index_ != nullptr && kv_ssd_ != nullptr) {
        if (status == nullptr) {
            for (int i = 0 ; i < 64 ; i ++) {
                printf("may be error!!!!!!!!!!!!!!! %s status is nullptr\n", __func__);
            }
            fflush(stdout);
        }
            kv_worker_->kvEnqueue((char *) du_key.data(), 16, 0, (char *) du_value.data(), du_value.size());
    
            //auto ssd_start = clock_->NowMicros();
            status[writer_id] = 2;
            kv_ssd_->kv_put_buffer_async(du_key.data(), du_key.size(), du_value.data(), du_value.size(), (int *) nullptr); 
            //auto ssd_end = clock_->NowMicros();
            //uint64_t ssd_lat = ssd_end - ssd_start;
            //RecordTimeToHistogram(stats_, KV_WRITE, ssd_lat);
     
        //auto index_start = clock_->NowMicros();
        kv_index_->keyInsert((char *) du_key.data(), 16, sequence, du_value.size());  
        //auto index_end = clock_->NowMicros();
        //uint64_t index_lat = index_end - index_start;
        //RecordTimeToHistogram(stats_, KV_WRITE_INDEX, index_lat);
       
        return true;
    } else {
        printf("kv_ssd or index is nullptr\n");
        return false;
    }
}
...
size_t WriteThread::EnterAsBatchGroupLeader(Writer* leader,
                                            WriteGroup* write_group, volatile int * status) {
  ...
  /*
  if (kv_ssd_enable_ == 0) {
      auto batch_size = WriteBatchInternal::ByteSize(leader->batch);
      char * buffer = (char *)WriteBatchInternal::Contents(leader->batch).data();
      size_t value_size = (size_t) (batch_size - 32);
      Slice du_key(buffer + 14, 16);
      Slice du_value(buffer + 32, value_size);
      kv_worker_->kvEnqueue((char *) du_key.data(), 16, 0, (char *) du_value.data(), du_value.size());
  }
  */
  if (kv_ssd_enable_ == 4) {
      ...
      Slice du_key(buffer + 14, 16);
      Slice du_value(buffer + 32, value_size);
      kv_worker_->kvEnqueue((char *) du_key.data(), 16, leader->sequence, (char *) du_value.data(), du_value.size());
      */
  } else if (WriteBatchInternal::ByteSize(leader->batch) >= 4096 && kv_ssd_enable_ == 3) {
      if (kv_ssd_ != nullptr) {
          uint64_t wr_st = clock_->NowMicros();
          WriteToKVSSD((char *)WriteBatchInternal::Contents(leader->batch).data(), 16, (int)WriteBatchInternal::ByteSize(leader->batch) - 32, (int) WriteBatchInternal::Sequence(leader->batch), writer_id ++, status);
          uint64_t wr_end = clock_->NowMicros();
          suc_kv = true;
          wr_to_kv += (wr_end - wr_st);
      }
  } else if (kv_ssd_enable_ == 3) {
      uint64_t check_st = clock_->NowMicros();
      CheckKVSSD((char *)WriteBatchInternal::Contents(leader->batch).data(), 16);
      uint64_t check_end = clock_->NowMicros();
      suc_check = true;
      check_kv += (check_end - check_st); 
  }
  ...
  while (w != newest_writer) {
    ...
    if (batch_size >= 4096 && kv_ssd_enable_ == 3) {
    //if (batch_size >= 4096 && kv_ssd_enable_ == 3) {
        uint64_t wr_st = clock_->NowMicros();
        WriteToKVSSD((char *)WriteBatchInternal::Contents(w->batch).data(), 16, batch_size - 32, (int) WriteBatchInternal::Sequence(w->batch), writer_id++, status);
        uint64_t wr_end = clock_->NowMicros();
        wr_to_kv += (wr_end - wr_st);
        suc_kv = true;
        du_cnt[2]++;
        ...
        uint64_t check_st = clock_->NowMicros();
        CheckKVSSD((char *)WriteBatchInternal::Contents(w->batch).data(), 16);
        uint64_t check_end = clock_->NowMicros();
        suc_check = true;
        check_kv += (check_end - check_st); 
    }
  ...
  if (suc_kv) {
      RecordTimeToHistogram(stats_, KV_WRITE_TO_KVSSD, wr_to_kv);
  } 
  if (suc_check) {
      RecordTimeToHistogram(stats_, KV_CHECK_KVSSD, check_kv);
  }
  //if (du_skip) printf("cnt %d %d %d group size %ld \n", du_cnt[0], du_cnt[1], du_cnt[2], write_group->size);
  ...
  if (false) {
      if (!allow_concurrent_memtable_write_ || !leader->batch->HasMerge()) {
          ...
          while (w != newest_writer) {
              ...
              /*
              if (kv_ssd_ != nullptr && (kv_ssd_enable_ == 1 || kv_ssd_enable_ == 2)) {
                  if (WriteBatchInternal::ByteSize(leader->batch) >= 4096) {
                      continue;
                  }
              }
              */
