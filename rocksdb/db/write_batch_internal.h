...
class WriteBatchInternal {
 public:
  ...
  static Status InsertInto(
      ...
      bool seq_per_batch = false, bool batch_per_txn = true, uint8_t direction = 0);
  ...
  static Status InsertInto(WriteThread::Writer* writer, SequenceNumber sequence,
                           ...
                           bool hint_per_batch = false, uint8_t direction = 0);
  ...
  static Status Iterate(const WriteBatch* wb, WriteBatch::Handler* handler,
                        size_t begin, size_t end, uint8_t direction=0);
...
class TimestampUpdater : public WriteBatch::Handler {
 public:
  ...
  Status PutCF(uint32_t cf, const Slice& key, const Slice&, uint8_t direction = 0) override {
      if (direction) {}
