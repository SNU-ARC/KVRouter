...
Status CompactionJob::Run() {
  ...
  int du_level = compact_->compaction->level(); 
  int du_input_level = compact_->compaction->start_level() ;
  int du_output_level = compact_->compaction->output_level() ;
  int du_start_files = vstorage->NumLevelFiles(du_input_level);
  int du_output_files = vstorage->NumLevelFiles(du_output_level);
  ...
  std::cout << " DU : " << __func__ << "\t" <<
      du_start_files << "\t" <<
      du_output_files << "\t" <<
      vstorage->NumLevelFiles(du_input_level) << "\t" <<
      vstorage->NumLevelFiles(du_output_level) << "\t" <<
      du_level << "\t" <<
      du_input_level << "\t" <<
      du_output_level << "\t" <<
      std::endl;
  ...
  for (int i = 0 ; i < 4 ; i ++) {
    printf("My level %d Files %lld Bytes %lld \n", i , (long long int)  vstorage->NumLevelFiles(i), (long long int)  vstorage->NumLevelBytes(i)); 
  }
...
int CompactionJob::CheckKVAsync (volatile int * complete_status, int tail, int head, bool last) {
    uint64_t start_micros = (uint64_t) db_options_.clock->NowMicros();
    int tail_l = tail;
    int head_l = head;
    while (tail_l != head_l) {
        // submit not complete
        if (complete_status[head_l] == 2) {
            // Wait until done !
...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
  ...
  bool finish = false;  
  int test_qwer_value = 0;
  int ret_tail = 0;
  int req_cnt = 0;
  bool dump_to_kvssd = false;
  std::vector<int> async_io_status;
  printf("compaction enable %d bottommost %d kv_comp_enable %d output level %d \n", (int)kv_ssd_->isCompactionEnable(), (int)compact_->compaction->bottommost_level(), (int)kvssd_compaction_enable_, kv_ssd_->getLastLevel());
  if (kv_ssd_->isCompactionEnable() && compact_->compaction->output_level() == kv_ssd_->getLastLevel() && kvssd_compaction_enable_ && (kv_ssd_enable_ == 3 || kv_ssd_enable_ == 4)) {
#if (0)
      test_qwer_value = kv_ssd_->getScore();
      int large_number = 0;
      int small_number = 0;
      
      std::cout << "Smallest User Key from DU " << compact_->compaction->GetSmallestUserKey().ToString(false) << std::endl; 
      std::cout << "Largest User Key from DU " << compact_->compaction->GetLargestUserKey().ToString(false) << std::endl; 
       
      char large_char[16];
      char small_char[16];
      memcpy(large_char, (void *) compact_->compaction->GetLargestUserKey().data(), 12);
      memcpy(small_char, (void *) compact_->compaction->GetSmallestUserKey().data(), 12);
      large_char[12] = '\0';
      small_char[12] = '\0';
      sscanf(large_char, "%d", &large_number); 
      sscanf(small_char, "%d", &small_number); 
        
      printf("level %d small number %d large number %d test %d \n", compact_->compaction->output_level(), small_number, large_number, test_qwer_value);
        
      // small = 0 large 900 test 100
      // in this case only (small <= test) 
      if (small_number <= test_qwer_value) {
          printf("Sucess to Dump !!!!!!!!!! \n");
          ret_tail = kv_comp_->compPrepare(compact_->compaction->start_level(), (char *)compact_->compaction->GetSmallestUserKey().data(), (char *) compact_->compaction->GetLargestUserKey().data(), 16);
          dump_to_kvssd = true;
          if (dump_to_kvssd && compact_->compaction->output_level() == 4) {
              printf("Befor Start Compaction ret tail %d\n", ret_tail);
              auto vstorage = cfd->current()->storage_info();
              for (int level = 0; level <= compact_->compaction->output_level(); ++level) {
                  std::cout << "level " << level << " files " << vstorage->NumLevelFiles(level) << " Bytes " << vstorage->NumLevelBytes(level) << std::endl;
              }
          }
      } else {
      }
      ...
      ret_tail = kv_comp_->compPrepare(compact_->compaction->start_level(), (char *)compact_->compaction->GetSmallestUserKey().data(), (char *) compact_->compaction->GetLargestUserKey().data(), 16);
      dump_to_kvssd = true;
      if (dump_to_kvssd && compact_->compaction->output_level() == kv_ssd_->getLastLevel()) {
          printf("Befor Start Compaction ret tail %d\n", ret_tail);
          auto vstorage = cfd->current()->storage_info();
          for (int level = 0; level <= compact_->compaction->output_level(); ++level) {
              std::cout << "level " << level << " files " << vstorage->NumLevelFiles(level) << " Bytes " << vstorage->NumLevelBytes(level) << std::endl;
  ...
  while (status.ok() && !cfd->IsDropped() && c_iter->Valid()) {
    ...
    /*
    if (c_iter->value().size() >= 4096 && (kv_ssd_enable_ == 1 || kv_ssd_enable_ == 2 || kv_ssd_enable_ == 3 || kv_ssd_enable_ == 4)) {
        if (rand() % 10000 == 0)
            std::cout << "fucking why large value? " << c_iter->key().data() << std::endl;
    }
    */
    ...
    if (dump_to_kvssd && !finish) {
        if (false) {
        } else {
            uint64_t compare_start = (uint64_t) db_options_.clock->NowMicros();
            char * du_key = (char *) c_iter->key().data();
            char compare_key[17];
            memcpy(compare_key, du_key, 12);
            compare_key[12] = '\0';
            int compare_value = 0;
            sscanf(compare_key, "%d", &compare_value); 
            uint64_t compare_end = (uint64_t) db_options_.clock->NowMicros();
            RecordTimeToHistogram(stats_, KV_COMPACTION_COMPARE, compare_end - compare_start);

            if (req_cnt && ret_tail) {}
            bool kv_write = false;
            //if (c_iter->value().size() >= 900) {
            //if (c_iter->value().size() >= kv_worker_->getCompSize()) {
            //if (c_iter->value().size() >= (size_t) kv_worker_->getCompSize() && ((compare_value % 100) < kv_worker_->getCompRatio())) {
            if ((c_iter->value().size() > (size_t) kv_worker_->getCompSize()) ||
                (c_iter->value().size() == (size_t) kv_worker_->getCompSize() && ((compare_value % 100) < kv_worker_->getCompRatio()))) {
            
            
            //if (c_iter->value().size() >= 1024 && compare_value < test_qwer_value) {
        ...
        /*
        if (warmup_phase_ != 1) {
            std::cout << "fucking" << c_iter->key().data() << std::endl;
            fflush(stdout);
        }
        */
        status = sub_compact->AddToOutput(*c_iter, open_file_func, close_file_func);
  ...
  uint64_t compaction_end = (uint64_t) db_options_.clock->NowMicros();

  if (dump_to_kvssd) {
    uint64_t finish_start = (uint64_t) db_options_.clock->NowMicros();
    // Wait until async write done !
    /*
    if (async_tail % 512 == 0) {
        for (int i = async_tail ; i < (async_tail + 512) ; ) {
            if (complete_status[i] == 2) {
                continue;
            }
            complete_status[i] = 0;     
            i++;
    ...
    */
    CheckKVAsync(complete_status, async_tail, async_head, true);
    kv_comp_->sulFlush(0, 0, 3);  // last is level !!!!!!! TODO 

    kv_comp_->compDone(ret_tail ,16);
    printf("Success dump to kvssd\n");
    fflush(stdout);
    free((void *) complete_status);
    uint64_t finish_end = (uint64_t) db_options_.clock->NowMicros();
    RecordTimeToHistogram(stats_, KV_COMPACTION_FINISH, finish_end - finish_start);
    RecordTimeToHistogram(stats_, KV_COMPACTION_KVSSD, compaction_end - compaction_start);
  } else {
    RecordTimeToHistogram(stats_, KV_COMPACTION_NORMAL, compaction_end - compaction_start);
  }
  printf("my compaction level input %d output %d is last level %d bottom %d \n", 
          compact_->compaction->start_level(),
          compact_->compaction->output_level(),
          (int) compact_->compaction->is_last_level(),
          (int) compact_->compaction->bottommost_level());

  std::cout << c_iter_stats.total_blob_bytes_read << "\t" <<
      c_iter_stats.num_input_deletion_records << "\t" <<
      c_iter_stats.total_input_raw_key_bytes  << "\t" <<
      c_iter_stats.total_input_raw_value_bytes  << std::endl;
  ...
  if (dump_to_kvssd && compact_->compaction->output_level() == 4) {
      printf("Befor Start Compaction ret tail %d\n", ret_tail);
      printf("Write RocksDB %ld Write KV SSD %ld\n", rocks_bytes, kv_bytes);
      auto vstorage = cfd->current()->storage_info();
      for (int level = 0; level <= compact_->compaction->output_level(); ++level) {
          std::cout << "level " << level << " files " << vstorage->NumLevelFiles(level) << " Bytes " << vstorage->NumLevelBytes(level) << std::endl;
          /*
          ...
          for (const auto& meta : level_files) {
              ...
              std::cout << "small " << meta->smallest.user_key().ToString(false) <<  " Large " << meta->largest.user_key().ToString(false) << std::endl;
          }
          */
      }
  }
