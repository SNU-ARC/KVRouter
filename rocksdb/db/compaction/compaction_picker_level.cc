...
void LevelCompactionBuilder::SetupInitialFiles() {
  ...
  for (int i = 0; i < compaction_picker_->NumberLevels() - 1; i++) {
    ...
    printf ("level %d score %f\n", (int)start_level_, (float) start_level_score_);
    if (start_level_score_ >= 1) {
      ...
      printf("After picked file to comapct result %d\n", (int) picked_file_to_compact);
...
bool LevelCompactionBuilder::SetupOtherInputsIfNeeded() {
  ...
  if (output_level_ != 0) {
    ...
    printf("\t\tInput Level input push back here !\n");
    compaction_inputs_.push_back(start_level_inputs_);
    if (!output_level_inputs_.empty()) {
      compaction_inputs_.push_back(output_level_inputs_);
      printf("\t\tOutput Level input push back here !\n");
    }
...
bool LevelCompactionBuilder::PickFileToCompact() {
  ...
  // level_files => input fiiles
  // std::cout << level_files << std::endl;
  ...
  for (cmp_idx = vstorage_->NextCompactionIndex(start_level_);
    ...
    // narrower output range because we do not need to read ...
    // 0 : not kv compaction   1 : enable but not set region  2 : enable and set region ( key 100 - 300 )
        /*
    kv_compaction_status = 0;
    if (kv_comp_) {
        // kv compaction mode is enabled! 
        if (kv_comp_->enabled()) {
            // return true if range is not set or inbound! 
            char * smallest_p = (char *) smallest.user_key().data();
            char * largest_p = (char *) largest.user_key().data();
            if (!kv_comp_->isRangeSet()) {
                kv_compaction_status = 1;
            } else if (kv_comp_->checkRange(smallest_p, largest_p, 16)) {
                kv_compaction_status = 2;
            } 
        }
    }
        */
  ...
  // kv compaction checker!  
  if (kv_compaction_status != 0) {
      //FielMetaData * output_meta_files = output_level_inputs.files; 
      // Maybe not need! expand range need to setup after compaction! 
      /*
      int output_file_size = output_level_inputs.files.size();
      char smallest_p = (char *) output_level_inputs.files[0].smallest.user_key().data();
      char largest_p = (char *) output_level_inputs.files[output_file_size-1].largest.user_key().data();
      kv_comp_->expandRange(smallest_p, largest_p);
      */
  }
